...
- 페이지네이션은 프로젝트 전체에서 한 방식으로 통일 — **커서 방식** ([ADR-0012](docs/decisions/0012-list-pagination-cursor.md), 2026-09-01 accepted. notification-list 가 최초 도입). shape: `?cursor={lastId}&size=N` → **nullable 절대값 `nextCursor`**(`null` = 끝). 🔴 **`hasNext` 불리언 금지** — 불리언을 주면 앱이 *"마지막 항목의 id 를 커서로 쓴다"* 는 규칙의 사본을 갖고, 서버가 커서 표현을 바꾸는 날 그 사본이 조용히 틀린다. 🔴 **정렬·커서 키는 `id DESC`, `created_at` 이 아니다** — `created_at` 은 엔티티가 실어 보내는 값이라 시계 보정에 밀리면 삽입 순서와 어긋나고, 커서가 그 위에 서면 **어긋난 구간이 예외도 로그도 없이 건너뛰어진다.** **표시는 시각, 순서는 id** — 응답의 `createdAt` 은 화면에 그리라고 있는 것이지 정렬하라고 있는 것이 아니다. ⚠️ **기존 목록 6종은 지금 따라 고치지 않는다** — 미완성이 아니라 결정이다(사유는 ADR §적용 범위). **적용 확대의 트리거는 규모 신호이지 이 규칙의 존재가 아니다.** 단 **머리에 append 되는 목록(피드·타임라인·알림류)은 규모 신호를 기다리지 않는다** — offset 의 실패가 규모와 무관하기 때문이다. 정렬 축을 검증할 때는 **두 축을 갈라놓는 시드**를 써라(ADR §차등 검증) — 안 그러면 그 테스트는 아무것도 증명하지 않는다.
