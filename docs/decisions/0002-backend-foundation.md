...
| 401 | 토큰 만료 | Refresh Token으로 재시도 — ⚠️ 아래 실측 정정 참조 |
...
> 🔴 **실측 정정 (2026-08-26, loser-ranking 중 backend-dev 발견)**: **인증 실패는 이 규약의 예외다 —
> 실제로는 bare HTTP 401 이 나간다.** `UnauthorizedEntryPoint` 가 `GlobalExceptionHandler` 에 도달하기
> **전인 Security 필터 단계**에서 직접 응답을 쓰기 때문이며, 기존 `GET /record` 등 전 엔드포인트 공통
> 동작임을 바이트 대조로 확인했다. 위 표의 "code=401" 행은 그 의미로 실현된 적이 없다.
> **이것이 올바른 동작이다** — 모바일 Ktor `Auth(bearer)` 플러그인의 자동 토큰 갱신은 **HTTP status
> 401 에 반응**하므로(ADR-0009), body code 방식이었다면 자동 갱신이 아예 동작하지 않았다.
> 문서가 틀렸고 서버·앱이 맞았다. 각 feature 계약에 복사된 "항상 HTTP 200" 문구의 일괄 감사는
> 백로그 등재.
