...
> ⚠️ **2026-08-03 정정**: `soul-oath` 착수 시 같은 방식(`@Test` 줄 수)으로 잰 `HEAD` 값은 **144**다. **148의 근거를 재현하지 못했다. 원인 미상.**
> **회귀는 아니다** — 삭제된 테스트 파일 0건이고, 이후 사용자 커밋 3개(`4cad47e`/`e450a97`/`6a3de0a`) 전부 `@Test` = 144로 동일해 리팩터가 건드린 것도 아니다.
> pm-lead가 "XML 집계가 `commonTest`를 두 번 셌을 것"이라는 가설을 냈으나 **mobile-dev가 실측으로 반증했다** — 같은 이름의 XML이 두 task에서 나오는 경우 0건이고, 중복이었다면 `soul-oath` 시점 총계가 210이 아니라 214여야 한다. **그럴듯한 원인을 적어두면 그게 다음 사람의 틀린 출발점이 되므로 "원인 미상"으로 남긴다.**
