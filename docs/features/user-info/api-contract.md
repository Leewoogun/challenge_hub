...
## 시간 포맷
**`yyyy-MM-dd HH:mm:ss` (KST 벽시계)** — [ADR-0010](../../decisions/0010-datetime-model-localdatetime.md). **ISO-8601 이 아니다** (`T`·`Z`·offset·밀리초 없음). 날짜 전용은 `yyyy-MM-dd`.

> 본 endpoint 응답에는 시간 필드가 없다. 그래도 규칙을 적어두는 이유는 **다음 사람이 "다른 계약은 뭐라고 적었나" 하고 이 문장을 복사하기 때문**이다 — 실제로 2026-08-06 까지 여기 `ISO-8601 UTC` 가 남아 있었고, 시간 필드가 없다는 괄호가 **문서가 프로젝트 규칙을 틀리게 말한다는 사실을 바꾸지 못했다.**
...
| 2026-08-06 | 🔴 시간 포맷 절 정정 (`confirmed` 후) | backend-dev. *"ISO-8601 UTC. (본 endpoint 응답에 시간 필드 없음.)"* → ADR-0010 형식. **괄호가 구해주지 못한다** — 시간 필드가 없다는 사실이 *문서가 프로젝트 규칙을 틀리게 말한다*는 것을 바꾸지 않고, 이건 정확히 **복사되는 종류의 문장**이다(다음 사람이 "다른 계약은 뭐라고 적었나" 하고 보는 자리). #29 린트 경고 전수 분류에서 발견 |
