...
- **작성자**: mobile-dev
- **작성일**: 2026-04-24 (스켈레톤) / 2026-05-07 (Android+iOS SDK 실연동) / **2026-05-11 (디자인 2차 반영 완료)**
- **상태**: ✅ 코드 작업 종료. 운영성 잔여(실 단말 E2E smoke, Docker 통합 테스트, 시각 검증 등)는 backlog.md에서 추적.

---

## 2026-05-11 — LoginScreen 디자인 2차 반영 완료

placeholder UI("맹세" 텍스트 + 슬로건 + 카카오 옐로우 버튼)를 design.md 결정 사항(2026-05-08 pm-lead 확정 9건)과 Lovable 갱신본(`challenge-design/oathbound-challenges/src/routes/login.tsx`)에 맞춰 풀세트로 교체.

### 구현 요약

`feat/login-design-pass` 브랜치에서 mobile-dev가 1차 작업(`fb68069` — 단일 LoginScreen 인라인 + SoulStampLogo 추출), 이후 사용자가 직접 컴포넌트 분할 리팩터(`d7529cc` — 6개 component 파일 + 디자인시스템 Divider/Spacer 추출). main에 fast-forward 머지 후 origin push 완료.

### 사용한 모바일 레포 스킬

- `modify-feature/SKILL.md` — 기존 LoginScreen UI 교체에 적용 (api/domain/data 변경 없음)
- `design-system/SKILL.md` (간접) — `:core:designsystem`에 신규 재사용 컴포넌트(StatusPillBadge, LabeledDivider, Divider, Spacer) 추가

### 변경된 파일

**신규 — `:core:designsystem`** (재사용 컴포넌트):
- `core/designsystem/src/commonMain/.../components/StatusPillBadge.kt` — 점 + 텍스트 캡슐 뱃지 (primary 톤 기본, "SOUL CONTRACT" 용)
- `core/designsystem/src/commonMain/.../components/LabeledDivider.kt` — 양쪽 1px 구분선 + 중앙 라벨
- `core/designsystem/src/commonMain/.../components/Divider.kt` — 표준 Divider 래퍼
- `core/designsystem/src/commonMain/.../components/Spacer.kt` — 표준 Spacer 래퍼

**신규 — `:feature:login/component`** (Hero/CTA 분할 composable):
- `feature/login/src/commonMain/.../component/SoulStampLogo.kt` — 140dp fire-gradient 스탬프 + 내부 ring + Flame(56dp) + Sparkles 2개. pulse-fire 맥동 + sparkle 깜빡임. **외곽 회전 링은 결정대로 제거.**
- `feature/login/src/commonMain/.../component/BackgroundDecor.kt` — 상단 글로우(500dp, opacity 30%) + 하단 fire 블롭(400dp, opacity 20%)
- `feature/login/src/commonMain/.../component/ChallengeSection.kt` — Hero 컬럼(스탬프 + 뱃지 + 타이틀 + 서브 카피)
- `feature/login/src/commonMain/.../component/ChallengeTitle.kt` — "영혼을 걸어라. 🔥" — "영혼"만 `ChallengeBrushes.fire` brush 텍스트, 🔥 wiggle 애니메이션
- `feature/login/src/commonMain/.../component/FooterAgreementText.kt` — 약관/개인정보처리방침 강조 풋터
- `feature/login/src/commonMain/.../component/LoginButtonSection.kt` — LabeledDivider + 카카오 버튼(`BrandColors.KakaoYellow`/`KakaoLabel`, 52dp 높이, 16dp radius, 텍스트 only "카카오로 시작하기")

**수정**:
- `feature/login/src/commonMain/.../LoginScreen.kt` — placeholder 전체 교체. 배경 BackgroundDecor + Hero ChallengeSection + CTA LoginButtonSection + Footer FooterAgreementText. slide-up/fadeIn 진입 애니메이션(CTA 0.2s delay). 로딩 오버레이(검정 30% + CircularProgressIndicator) 유지.
- `feature/login/build.gradle.kts` — `compose.materialIconsExtended` 의존성 추가 (`LocalFireDepartment`, `AutoAwesome` 사용)

### 빌드 결과

- `:composeApp:linkDebugFrameworkIosSimulatorArm64` — **BUILD SUCCESSFUL** (28s)
- `:composeApp:compileDebugKotlinAndroid` — **BUILD SUCCESSFUL** (20s)
- `:feature:login:compileCommonMainKotlinMetadata` — **BUILD SUCCESSFUL** (13s)
- `:core:designsystem:compileCommonMainKotlinMetadata` — **BUILD SUCCESSFUL** (28s)

`materialIconsExtended` 1.7.3 핀 사용으로 deprecation 경고 발생(Kotlin 2.2.20 / CMP 1.10.3 호환 이슈) — Material Symbols 마이그레이션 ADR 후보.

### 테스트 결과

- **LoginViewModelTest (commonTest): 4/4 passed** (Android Unit + iOS SimulatorArm64). UI 변경만이라 ViewModel 회귀 0.
- **시각 검증 미수행** — Compose UI 자동 검증 불가. 사용자가 Android 에뮬레이터/iOS Simulator에서 직접 확인 필요(backlog 등재).

### 브랜치 / 커밋

- 브랜치: `feat/login-design-pass` (작업) → `main`에 fast-forward 머지 → origin/main push 완료
- 커밋: `fb68069` (mobile-dev 1차) + `d7529cc` (사용자 컴포넌트 분할 리팩터)

### 후속 검토 4건 (backlog 등재)

1. **Blur 미적용**: Lovable `blur-3xl`(48px) 등가가 Compose에 없어 alpha만으로 근사. 시각 차이 크면 `graphicsLayer { renderEffect = BlurEffect(...) }` 도입(Android API 31+ / KMP 호환성 검증 필요).
2. **Material Icons Extended deprecation**: `materialIconsExtended` 1.7.3 핀에 deprecation 경고. Material Symbols(vector resources)로 마이그레이션 ADR 후보.
3. **Glow shadow / 외광 효과**: tokens.md §3 `--shadow-glow`(0 0 30px primary 20%)는 Compose 기본 `shadow()`로 표현 어려움. Canvas glow 별도 작업 후보.
4. **카카오 버튼 ripple 색**: design.md의 `BrandColors.KakaoYellowPressed` ripple 매핑 별도 작업 — 디자이너 확인 후.

---

## 2026-04-24 — 초기 스켈레톤 구현
