...
| ⑫ | 🔴 **1위 패배율이 바로 아래 순위보다 낮게 보인다** — 실서버 데이터에서 1위 `4패 · 패배율 80%` 위에 2위 `3패 · 패배율 100%` 가 붙는다(rank-mobile 발견, **개발 DB 첫 화면에서 즉시 관측**). 정렬은 계약대로 정상(`losses` 1차 키)인데, **캡션이 패배율을 병기해서 순위 근거와 어긋나 보인다** | **① 현행 유지 — pm-lead 판정(2026-08-26)** (§7.1) |

### §7.1 ⑫ 선택지 — 디자이너가 실화면을 보고 고를 것

| | 안 | 성격 |
|---|---|---|
| **①** | **현행 유지** — 캡션에 `{losses}패 · 패배율 {lossRate}%` 를 그대로 병기 | ✅ **채택(기본값).** 헤더 부제 *"패배의 왕좌 — 많이 진 놈이 대장"* 이 **정렬 기준을 이미 밝히고 있다.** 정본 그대로라 이탈 0 |
| **②** | **캡션에서 패배율을 뺀다** — `{losses}패` 만 | 🔴 **정본 이탈.** 어긋나 보이는 원인을 없애지만, 패배율은 *"몇 판 중에 졌나"* 를 주는 유일한 맥락이라 잃는 것이 있다(3전 3패와 30전 3패가 같아 보인다) |
| **③** | **기준 명시 문구 추가** — 명단 섹션 제목 옆 등에 *"총 패배 순"* | 정본 이탈이 작다. 다만 헤더 부제와 **같은 말을 두 번** 하게 된다 |

🔴 **재정렬로 "고치는" 방향은 없다.** 패배율 순으로 다시 세우는 것은 **계약 위반**이다 — 정렬과 `rank`
부여는 서버 몫이고([api-contract](./api-contract.md) 쟁점 ①, `losses` 1차 키 `confirmed`), 앱이
다시 세우면 서버가 준 `rank` 와 화면 순서가 갈린다. **앱 테스트가 이미 이 방향을 막고 있다.**
이 항목은 **표시 문제지 정렬 문제가 아니다.**
...
| 일시 | 변경 |
|------|------|
| 2026-08-26 16:24:18 | **§7 확인 대상 ⑫ 등재 + §7.1 신설** (pm-lead 지시). **1위 패배율이 하위 순위보다 낮게 보이는 병치** — 실서버 1위 `4패 · 80%` 바로 아래 2위 `3패 · 100%`(rank-mobile 발견, 개발 DB 첫 화면에서 즉시 관측). 정렬은 계약대로 정상(`losses` 1차 키)이고 **캡션이 패배율을 병기해 순위 근거와 어긋나 보이는 표시 문제**다. **pm-lead 판정: ① 현행 유지** — 헤더 부제 *"많이 진 놈이 대장"* 이 정렬 기준을 이미 밝히고, ②(캡션에서 패배율 제거)는 정본 이탈이다. 디자이너가 실화면을 보고 뒤집을 수 있게 **선택지 3안을 §7.1 에 병기**(①현행 / ②패배율 제거 / ③기준 명시 문구). 🔴 **재정렬로 "고치는" 방향은 계약 위반**(정렬·`rank` 부여는 서버 몫, 앱 테스트가 이미 막고 있음)임을 명시 — *"표시 문제지 정렬 문제가 아니다"*. |
