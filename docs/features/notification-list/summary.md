...
- **완료일**: 2026-09-01
- **상태**: **implemented** — 전 범위 구현·검증 완료 + ✅ **양 레포 커밋**(2026-09-05 실측: app `b5e7233` / server `bc18aea`). 🔴 **`completed` 가 아닌 사유는 이제 실기 미확인 하나다**
...
### 7. 타입 문자열 사본 0건 — 보장 방식이 "사본 없음"이 아니다
**사본 금지가 이 feature 를 관통한 주제였다** — `toRoute()` 를 `:core:push` 로 승격한 것(T-M2),
그리고 스켈레톤 1차에서 **실제로 난 사본을 걷어낸 것**(`NotificationTypeVisual` 에 타입 리터럴 4개가
생겼다가 `PushEvent` 상수 참조로 교체).
🔴 **보장은 *"사본이 0건임"* 을 검증하는 게 아니라 `PushEvent.TYPE_*` ↔ 서버 wire 값의 리터럴
일치를 고정하는 쪽이다** — 사본이 없으니 **한 벌만 맞으면 전부 맞는다.** 상수가 조용히 바뀌어도
폴백으로 떨어질 뿐 **아무것도 실패하지 않는 자리**라 명시 고정이 필요하다.

## 미해결 이슈

| # | 내용 | 담당 |
|---|---|---|
| 1 | ~~🔴 **양 레포 미커밋** — 서버 12파일 · 앱 34경로~~ ✅ **2026-09-05 해소** — app `b5e7233` / server `bc18aea`. 🔵 둘 다 **미푸시** | ~~user~~ |
