...
2. **실패 조건이 좁다.** *"자정을 넘기는 그 순간에 알림 목록을 띄워 놓고 보고 있을 것"* 이다.
   화면을 벗어났다 돌아오면 재구독이 일어나며 `now` 를 다시 잡는다 — **대부분의 경우 자동으로 맞는다**
   (§4.5.1 의 뱃지 소거가 기대는 것과 같은 구조다).
3. §3.4 의 *"초 단위로 갱신하지 않는다"* 와 같은 결이다. 목록은 **진입 시각 기준 스냅샷**이다.

→ **요구사항은 하나: 화면을 다시 열면 맞을 것.** 수단은 mobile 판단이다(§4.5.1 과 같은 형태로 낸다).
⚠️ **다만 `now` 가 ViewModel 생애당 1회가 아니라 구독당 1회여야 이 요구가 성립한다** — 전자면
앱을 며칠 켜둔 사용자에게 헤더가 계속 틀린다. 어느 쪽인지 확인해 달라.
