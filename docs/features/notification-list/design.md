...
└── content: LazyColumn(contentPadding = vertical 16dp)
    ├── (그룹마다) 날짜 헤더  "오늘"/"어제"/"M월 d일"           §3.6
    │   └── items { NotificationCard }                          §3 · §3.5
    └── 푸터: 인디케이터 | "다시 시도"  ← 헤더 없음             §6.3 · §3.6.4
...
## §3.5 🔴 탭 어포던스 — **비탭을 낮추지 않고, 탭에 신호를 더한다** (2026-09-01 2차 개정)

[spec 2차 개정](./spec.md)이 목록의 탭 규칙을 **타입 기준에서 상태 기준으로** 바꿨다. 그 결과
**비탭 카드가 소수가 아니라 다수가 된다** — 챌린지 한 건이 수락/거절 알림을 반드시 하나 만드는데
둘 다 비탭이다.
...
| 증거 도착 | ✅ 항상 |
| 수락 · 거절 | ❌ |
| 모르는 타입 · `referenceId == null` | ❌ (§3.2.2 · §7.2) |

### §3.5.1 🔴 이 절이 푸는 문제는 "충돌"이다

**`"탭 비활성"` 에는 이 문서에서 이미 다른 뜻이 있다.** §3.2.2 가 **모르는 타입**을
*무채색 아이콘 + 탭 비활성* 으로 명세했다. 여기에 수락·거절을 **같은 표현으로** 태우면
🔴 **정상 수신된 알림이 고장난 알림처럼 보인다** — 내용은 온전한데 색이 죽으면 사용자는
*"뭔가 잘못됐다"* 로 읽는다.

**해법: 두 축을 분리한다.** 비탭을 낮추는 대신 **탭 가능한 것에만 신호를 더한다.**

| 축 | 신호 | 무엇을 말하나 |
|---|---|---|
| **타입 축** | 아이콘 색 (의미색 ↔ 무채색) | *"이 알림의 종류를 아는가"* |
| **목적지 축** | chevron 유무 | *"갈 곳이 있는가"* |

두 축이 **독립**이라 겹치지 않는다. 모르는 타입은 **둘 다** 해당(무채색 + chevron 없음),
수락·거절은 **목적지 축만** 해당(색은 그대로). 지난 신청은 목적지 축만, 그것도 **시간에 따라** 해당.

🔴 **§7.2 를 뒤집는 것이 아니라 그 반대편을 채우는 것이다.** §7.2 는 여전히 유효하다 —
*"누를 수 없다는 것을 시각으로 표시하지 않는다"*(흐리게·톤다운 금지). 이 절은 **누를 수 있다는 것을
표시**한다. 같은 문장이 아니다.
...
| 요소 | 값 | 근거 |
|---|---|---|
| 기호 | `Icons.AutoMirrored.Filled.KeyboardArrowRight` **16dp** | [mypage §1.4.4](../mypage/design.md) 메뉴 카드와 **같은 값**. 앱에서 *"이 행은 어딘가로 간다"* 의 기존 어휘다 |
| 색 | `colorScheme.onSurfaceVariant` | 동상. 시각 캡션과 같은 톤 — **주인공이 아니다** |
| 위치 | `Row` 의 마지막, 🔴 **`Modifier.align(Alignment.CenterVertically)`** | Row 기본 정렬이 `Top`(§3)인데 chevron 만 세로 중앙으로 뺀다. 2줄 메시지에서 첫 줄에 붙으면 **카드 전체를 가리키는 기호가 한 줄만 가리키는 것처럼** 보인다 |
| 간격 | `Row` 의 `spacedBy(12.dp)` 를 **그대로 받는다** | 전용 값을 만들 근거가 없다 |
| 눌림 | 기존 구조 그대로 | `onClick != null` 일 때만 `clickable` 이라 리플이 자동으로 갈린다(`NotificationCard.kt:57`). **추가 작업 없음** |

🔴 **비탭 카드에 chevron 자리를 예약하지 않는다.** 텍스트 컬럼이 24dp 넓어질 뿐이다.
메시지가 좌측 정렬이라 **눈이 따라가는 왼쪽 모서리는 동일**하고, 없는 요소를 위해 빈 자리를 남기면
다음 사람이 *"여기 왜 비었지"* 를 묻는다.

### §3.5.3 🔴 같은 카드가 시간이 지나 탭 불가로 바뀌는 것 — **설명하지 않는다**

pm 이 명시로 물은 항목이다. 어제 눌리던 신청 알림이 오늘은 안 눌린다. **판정: chevron 의 부재가
신호의 전부고, 문구를 덧붙이지 않는다.**

1. 🔴 **알림은 사건의 기록이지 대상의 현재 상태가 아니다.** `"처리됨"`·`"만료됨"` 을 붙이는 순간
   카드가 **챌린지의 현재 상태를 주장**하게 된다. 그런데 목록은 **한 번 받아온 스냅샷**이라 그 주장을
   최신으로 유지할 수단이 없다 — 사용자가 홈에서 그 도전장을 수락하고 돌아오면 **그 문구가 거짓**이 된다.
   chevron 은 다르다. 그건 *"지금 이 카드를 누르면 갈 곳이 있나"* 라는 **화면 자신에 대한 사실**이라
   낡아도 기능만 어긋나지 거짓말은 안 한다.
2. **사용자는 전이를 목격하지 않는다.** 카드가 눈앞에서 바뀌는 게 아니다. **어제 탭한 것을 기억하고
   오늘 다시 시도할 때만** 인지되는데, 그때는 홈에서 그 도전장이 사라진 것을 이미 봤을 것이다.
3. 🔴 **설명 문구는 반드시 *"그래서 어떻게 됐는데?"* 를 유발하고, 이 화면은 답할 수 없다.**
   `"처리됨"` 은 수락인지 거절인지 만료인지 말하지 않는다. 말하려면 목록이 **챌린지 결과**를 알아야
   하는데 그건 홈·보관함의 일이다. → **없는 뜻을 지어내는 것**이고, §3.2.2 에서 세운 기준
   (*"의미를 모르는데 의미색을 주면 없는 뜻을 지어낸다"*)과 **같은 자리**다.

🔴 **명시적으로 기각하는 대안: 톤 다운.** 지난 신청을 흐리게 만드는 것은 §3.5.1 이 푼 충돌을
**그대로 되살린다.** 하지 마라.

#### 🔴 *"아이콘·색이 똑같고 화살표만 사라진다"* — **그게 의도다** (noti-mobile 확인 요청)

noti-mobile 이 지난 신청 카드에 대해 *"탭 가능하던 시절과 아이콘·색이 똑같고 화살표만 사라진다"* 고
짚고 **UiState 에 축을 하나 더 만들지 물었다. 필요 없다.** 지금 구조 그대로 간다.

**그 카드가 알리는 사건은 변하지 않았기 때문이다.** *"민수가 챌린지를 신청했다"* 는 어제도 오늘도
참이고, 아이콘·색은 **그 사건**을 그린다. 변한 것은 **목적지의 유무 하나**뿐이라 **목적지 축의 신호
하나만** 바뀌는 것이 정확하다.

🔴 **카드가 사건을 다르게 말하기 시작하면 그때부터 거짓이다.** 축을 하나 더 만들면 그 축이 무엇을
말할지 정해야 하는데, 말할 수 있는 것은 챌린지의 현재 상태뿐이고 그건 위 3가지 이유로 말하면 안 된다.

---

## §3.6 🔴 날짜 그룹핑 (2026-09-01 2차 개정)

목록을 날짜로 묶는다. 정본에 **날짜 그룹 헤더가 없다** — 앱 관례와 Lovable 의 **섹션 헤더 문법**에서
유도한다.

### §3.6.1 🔴 §3.4 의 다섯 가지 중 **두 가지가 헤더로 승격된다**

이 절의 핵심이다. 그룹 헤더가 날짜를 말하면 **행이 날짜를 다시 말할 이유가 없다** —
`어제` 헤더 아래 `어제` 행이 반복되면 중복이고, `9월 1일` 헤더 아래 `9월 1일` 행은 더 나쁘다.

| §3.4 가지 | 무엇을 말하나 | 2차 개정 후 자리 |
|---|---|---|
| ① `방금 전` · ② `N분 전` · ③ `N시간 전` | **시간** | **행에 남는다** (`오늘` 그룹 전용) |
| ④ `어제` · ⑤ `M월 d일` | **날짜** | 🔴 **헤더로 올라간다** |

→ **축이 깨끗하게 갈린다: 그룹 = 달력 축, 행 = 그 날 안에서 언제.**

| 그룹 | 행의 시각 표기 |
|---|---|
| `오늘` | **상대 시각** — §3.4 가지 ①~③ |
| `어제` · `M월 d일` | 🔴 **`HH:mm`**, **zero-pad** (예: `09:05` · `20:05`) |

#### ⚠️ `HH:mm` 은 새 방언이 아니다 — 앱에 선례가 있다 (noti-mobile 질의, 실측 확인)

noti-mobile 이 *"앱에 시:분 표기 선례가 있는지 확인 못 했다. 새 방언이 될 수 있다"* 고 물었다.
**있다.** `DeadlineAbsoluteText.kt:32-34` 가 마감 시각을 그릴 때 이미 zero-pad 된 시:분을 만든다:

```kotlin
val hour = deadline.hour.toString().padStart(2, '0')
val minute = deadline.minute.toString().padStart(2, '0')
return "${...monthNumber}/${...day} $hour:$minute"
```

→ **zero-pad 를 따른다**(`9:05` 아님 `09:05`). 같은 앱에서 시:분이 두 모양이면 그게 방언이다.

⚠️ **재사용은 불가능하고 관용구만 승계한다**(noti-mobile 실측). `deadlineAbsoluteText` 는 padding 이
`"M/d HH:mm"` 문자열 안에 박혀 있어 시:분만 떼어낼 수 없고, `ChallengeDateTimeFormats` 는 **wire 파싱
전용**이라 표시 함수를 넣을 자리가 아니다. → 신규 `toClockTimeText()`. 🔴 **같은 `padStart(2, '0')`
관용구를 쓴다** — 함수가 둘인 것과 표기가 둘인 것은 다르다.

⚠️ **다만 날짜 부분은 따라가지 않는다.** `deadlineAbsoluteText` 는 `M/d`(슬래시)를 쓰는데 이 문서의
그룹 헤더는 `{M}월 {d}일` 이다([mypage §2.4.3](../mypage/design.md) 승계). 🔴 **앱에 날짜 방언이 이미
둘이라는 뜻**이고, 그건 이 feature 가 만든 문제가 아니라 **물려받은 것**이다 — 여기서 `M/d` 로
갈아타면 보관함과 어긋나므로 **더 최근에 확정된 `{M}월 {d}일` 쪽에 붙는다.** §11-⑥ 에 등재.

⚠️ **이것이 §3.4 가지 ③의 "24시간" 경계를 무력화한다.** 어제 20:00 알림을 오늘 10:00 에 보면
§3.4 단독으로는 `"14시간 전"` 인데, 그룹핑 아래서는 `어제` 헤더 + `20:05` 다. **손실이 아니라 축
정리다** — 그룹이 달력을 맡으면 행이 24시간 산술을 겸할 이유가 없다.

🔴 **그렇다고 §3.4 를 잘라내지 마라.** 가지 ④·⑤ 는 `오늘` 그룹에서 **도달하지 않을 뿐 살아 있다.**
잘라내면 **③·④ 순서 논증을 고정한 반례 테스트가 함께 죽는데, 그 테스트가 그 함수의 유일한 방어**다.
그리고 그룹핑이 없는 다른 화면이 그 함수를 부를 수 있다.

### §3.6.2 그룹 헤더 규격 — 신규 토큰 0

정본에 날짜 그룹 헤더는 없지만 **섹션 헤더 문법은 있다** — `index.tsx:223` `ReceivedChallengesSection`
의 `text-sm font-bold text-foreground` + `mb-2 px-1`. [mypage §2.3.2 의 1차 본문](../mypage/design.md)이
같은 자리에서 이미 이 문법을 유도했다(그쪽은 월 이동 바와 **중복**이라 폐지됐고, **여기엔 중복
대상이 없다** — 이 목록은 실제로 여러 날짜에 걸친다).

| 요소 | 값 |
|---|---|
| 문구 | `오늘` / `어제` / `{M}월 {d}일` |
| 타이포 | `typography.bold14` / `colorScheme.onBackground` |
| 좌우 | 카드 gutter(20dp) + **4dp** = **24dp** (정본 `px-1`) |
| 상하 | 아래 8dp(정본 `mb-2`), 위 16dp (그룹 사이 숨) — **첫 그룹의 위 여백은 목록 `contentPadding` 이 낸다** |
| 배경 | **없음** (평면) |

- 🔴 **건수(`{n}건`)를 붙이지 않는다.** 정본 섹션 헤더에는 있지만, 거긴 **처리할 대상의 수**라
  행동을 바꾼다(받은 도전장 3건 → 3개를 처리해야 함). **지난 날짜의 알림 수는 아무 행동도 바꾸지
  않는다.** [mypage §2.3.2](../mypage/design.md) 가 건수를 살려 월 바로 옮긴 것과 다른 판단인데,
  거긴 *"이 달의 규모"* 가 탐색 축의 정보였기 때문이다. 여기 탐색 축은 스크롤이다.
- 🔴 **`stickyHeader` 를 쓰지 않는다.** ① 앱에 선례가 **0건**이다 ② 알림의 날짜 그룹은 **짧다**
  (하루 몇 건) — sticky 의 값은 **긴 그룹**에서 나오는데 여기선 헤더가 거의 항상 바뀌는 상태로 떠 있다
  ③ 상단바가 이미 고정 크롬이라 하나 더 얹으면 목록 가시 영역이 준다([mypage §2.3.2](../mypage/design.md)
  근거 3 승계).

### §3.6.3 🔴 커서 경계 — 같은 헤더가 두 번 찍히는 함정

**그룹을 페이지 단위로 만들지 마라.** 한 날짜가 페이지 경계에 걸치면(예: 1쪽 끝과 2쪽 시작이 둘 다
`9월 1일`) 페이지마다 헤더를 붙이는 구현은 **같은 헤더를 두 번** 찍는다.

- **헤더는 앱이 이어붙인 누적 목록 전체를 기준으로 계산한다.** UiState 가 평평한 누적 리스트를 갖고,
  렌더 직전에 그룹을 파생시킨다.
- 🔴 **계약에 그룹 필드를 넣지 마라.** 서버는 **페이지 단위로만** 아는 주체라 원천적으로 못 만든다 —
  넣으면 서버가 모르는 것을 아는 척하게 된다.

#### 🔴 자정 경계 — 타이머를 만들지 않는다 (noti-mobile 질의, 2026-09-01 판정)

noti-mobile 이 정확한 지점을 짚었다: *"행 시각과 같은 정책이라 일관되지만, **헤더는 `오늘` 이라는
단어라 어긋남이 더 눈에 띈다**"*. 맞는 지적이다 — `"3분 전"` 이 낡는 것과 `오늘` 이 틀리는 것은
**무게가 다르다.** 그럼에도 **타이머를 만들지 않는다.**

1. 🔴 **자정에 헤더를 갱신하면 목록이 눈앞에서 재구성된다.** `오늘` 그룹 전체가 `어제` 로 내려가고
   그 자리에 빈 `오늘` 이 생기거나 사라진다 — **읽던 자리가 통째로 밀린다.**
   낡은 헤더는 **수동적 부정확**이고 자정 재구성은 **능동적 방해**다. 후자가 더 나쁘다.
2. **실패 조건이 좁다** — 🔴 **단, 아래 §3.6.3.1 의 요구가 충족될 때만 그렇다.**
   *"자정을 넘기는 그 순간에 알림 목록을 띄워 놓고 보고 있을 것"* 으로 좁아지려면 **화면을 다시 열 때
   `now` 가 다시 잡혀야** 한다.
3. §3.4 의 *"초 단위로 갱신하지 않는다"* 와 같은 결이다. 목록은 **진입 시각 기준 스냅샷**이다.

→ **요구사항은 하나: 화면을 다시 열면 맞을 것.** 수단은 mobile 판단이다(§4.5.1 과 같은 형태로 낸다).

#### §3.6.3.1 🔴 이 요구는 **당시 성립하지 않고 있었다** — 실측으로 확인됨 (2026-09-01)

⚠️ **초판 §3.6.3 이 *"화면을 벗어났다 돌아오면 재구독이 일어나며 `now` 를 다시 잡는다"* 고 사실처럼
적었다. 실측 결과 거짓이었다.** 내가 붙인 확인 요청에 noti-mobile 이 답해 드러났다:

```kotlin
private var now: LocalDateTime? = null                  // NotificationViewModel.kt:60
val measuredNow = now ?: nowKst().also { now = it }     // :173  — 최초 1회만
init { loadFirstPage(); markAllAsRead() }               // 생애당 1회
```

**`now` 가 ViewModel 생애당 1회**였고, 알림 화면은 홈에서 push 로 열리는 하위 화면이라 상세를 다녀와도
**백스택에 남아 ViewModel 이 산다.** → 자정 한 번이 아니라 **앱을 켜둔 내내** 틀린다. 사흘 뒤에도
`오늘` 헤더가 사흘 전 그룹에 붙어 있고, 헤더뿐 아니라 **행의 상대시각/`HH:mm` 갈림까지 통째로** 틀린다.

🔴 **왜 초판에서는 안 보였나 — 낡음의 무게가 §3.6 으로 인해 승격됐다.**
그룹핑 전에는 낡아도 `"3시간 전"` 이 `"5시간 전"` 이 안 되는 **정도 문제**였다. 그룹핑이 붙으면서
같은 낡음이 **`오늘`/`어제` 라는 단정문**이 됐다. **§3.6 이 만든 새 조건**이고, §3.4 의 스냅샷 정책을
그대로 승계하면 되는 줄 알았던 것이 이 절의 오류다.

##### 🔵 다음에 정책을 승계할 때 쓸 2단 검사 (이 오류에서 뽑았다)

정책을 다른 자리로 옮길 때 **두 단계로 묻는다.** 두 문장이 다른 일을 하므로 **둘 다 남긴다.**

1. **질문** — *"이 정책이 견디던 하중이 새 자리에서도 같은가?"*
   승계는 정책만 옮기고 **하중은 안 본다.** 여기서 나는 §3.4 의 스냅샷 정책을 §3.6 으로 옮기면서
   그룹핑이 하중을 올려놓은 것을 안 봤다.
2. 🔴 **판정 기준** (pm-lead) — *"그 정책이 견디던 것이 **오차**인가 **문장의 참·거짓**인가?"*
   `"3시간 전"` → `"5시간 전"` 은 **오차**라 스냅샷이 견딘다. `오늘` 이 어제를 가리키는 것은
   **거짓**이라 안 견딘다. **값의 오차가 문장의 참·거짓으로 승격되는 순간이 정책을 다시 봐야 할
   지점**이고, 이 검사는 문서를 안 읽어도 코드에서 물을 수 있다.

🔵 **backend 가 독립적으로 같은 자리를 짚었다** — `referencePending` 을 로컬에 캐시하면 **오래된
`true` 로 만료된 신청의 탭을 허용**하게 된다(§7.2). **두 요구의 해법이 하나다**: 재진입마다 다시 받으면
`now` 도 `referencePending` 도 함께 새로워진다. 한쪽은 *"헤더가 거짓말한다"*, 다른 쪽은 *"만료된
신청이 눌린다"* 인데 **원인이 같은 캐시**다.

→ **해소**: 로딩을 `init` 에서 빼고 **화면 재진입마다 도는 자리**로 옮긴다(mobile 확정). `NavDisplay`
가 최상단 entry 만 composition 에 두므로 재진입 시 재구성되고 거기서 `now` 재측정 + 1쪽 재조회가 돈다 —
🔵 §4.5.1 의 뱃지 소거가 기대는 것과 **같은 구조**라 새 메커니즘이 아니다.

⚠️ **부수 효과 ① — 재진입마다 `read-all` 이 다시 돈다. 이건 규약에 맞다.** §4.3 의 *"목록을 열면 전부
읽음"* 이 그대로 적용될 뿐이고, 자리를 비운 사이 도착한 알림이 재진입 시 읽음 처리되는 것이 **오히려
정합**이다 — 반대로 첫 진입에만 처리하면 **사용자가 목록을 보고 있는데 뱃지가 켜져 있는** 상태가 된다.
🔴 지켜야 할 불변식은 *"쪽마다 다시 돌지 않는다"* 이지 *"생애당 1회"* 가 아니다.

#### §3.6.3.2 🔴 부수 효과 ② — **재진입하면 목록이 처음으로 돌아간다. 감수하고 적는다**

pm 이 명시로 물은 항목이다. 재진입 재로딩은 **1쪽만 남기고 스크롤을 맨 위로** 되돌린다. 3쪽쯤
내려가서 지난 증거 도착을 눌렀다 돌아오면 **맨 위**다. 그리고 이건 **주 왕복 경로**다 — 탭 가능한 것이
신청·증거 둘뿐이고 증거는 항상 탭 가능하다(§7.2).

🔴 **명세다. 버그가 아니다. 고치지 마라.** 적어 두지 않으면 다음 사람이 결함으로 보고 고치는데,
고치는 순간 §3.6.3.1 의 캐시 문제가 통째로 되살아난다.

##### 🔴 중간 안(1쪽만 재조회 + 나머지 유지)을 기각한 이유 — **ADR-0012 가 기각한 실패를 되살린다**

pm 이 제시한 2번 안은 *"새 1쪽 + 낡은 2~n쪽"* 을 이어붙인다. **그러면 같은 항목이 두 번 나온다.**
자리를 비운 사이 알림 2건이 도착하면 옛 1쪽의 마지막 2건이 새 1쪽에서 밀려나는데, **낡은 2쪽은 그
2건을 그대로 들고 있다.**

[ADR-0012 §근거 1](../../decisions/0012-list-pagination-cursor.md) 이 offset 을 기각한 사유가 정확히
이것이다 — *"머리에 새 행이 들어오는 동안 페이지가 밀려 2페이지에 1페이지 항목이 다시 나온다"*.
그리고 같은 ADR 이 **알림을 그 실패가 `상시` 조건인 도메인**으로 지목했다.
🔴 **커서를 고른 이유가 이 중복인데, 부분 재로딩이 커서 위에서 그 중복을 다시 만든다.**

그리고 신선도도 **부분적으로만** 참이 된다 — 1쪽 행은 `now`·`referencePending` 이 새롭고 2~n쪽은
낡은데, **화면에서 둘이 구분되지 않는다.** 전부 낡은 것보다 나쁘다. 어느 행을 믿을지 알 수 없다.

##### 감수의 대가가 작은 이유

- **탭 대상이 신선도 쪽에 쏠린다.** 증거 도착은 *"상대가 방금 인증했다"* 라 **위쪽에 있을 때** 누른다.
  3쪽 아래의 증거 도착은 이미 끝난 챌린지의 것이고(그래도 탭은 열려 있다 — 사진이 안 사라지므로),
  거기까지 내려가 누르는 사용은 이 화면의 주 용도가 아니다.
- **이 화면의 값은 spec 이 정의한 대로** *"푸시를 놓치면 그걸로 끝"* **을 메우는 것**이고, 놓친 푸시는
  목록 **머리**에 있다.

⚠️ **되돌아볼 조건**: 깊은 쪽의 알림을 탭하는 사용이 실제로 관측되면. 그때는 *"스크롤 위치 복원"* 이
아니라 **재로딩 자체를 다시 설계**해야 한다(§3.6.3.1 의 두 요구를 함께 지키면서). 🔴 **부분 재로딩으로
가지 마라** — 위에서 기각한 그대로다.

### §3.6.4 다른 상태와의 관계

- **§6.3 푸터**(다음 쪽 인디케이터 · `"다시 시도"`)는 **마지막 그룹 뒤**에 온다.
  🔴 **자기 헤더를 갖지 않는다** — 푸터는 알림이 아니다.
- **빈 상태(§5) · 첫 로딩 · 첫 조회 실패(§6.1)** — 그룹이 하나도 없다. 그대로다.
...
### §7.1 🔴 목적지 규칙의 사본을 만들지 마라 — ✅ **해소됨 (T-M2 완료)**

> ✅ **2026-09-01 — 아래 진단은 해소됐다. 기록으로 남긴다.**
> `toRoute()` 가 `MainViewModel` private → **`:core:push/PushEventRoute.kt:10` 으로 승격 + public 화**
> 됐고, `PushEvent.of(type, referenceId)` 가 신설돼 `from(Map)` 이 그리로 위임한다.
> 🔴 **사본을 만들 수 있었던 구조 자체가 없어진 것**이지 "사본을 안 만들기로 한" 것이 아니다 —
> 후자는 다음 사람이 뒤집을 수 있고 전자는 못 뒤집는다.
> ⚠️ **2차 개정이 이 위에 얹힌다**: 목적지 규칙은 그대로 하나인데, **목록만 그 앞에 탭 가능 여부를
> 한 겹 둔다**(§7.2). 🔴 **그 조건을 `toRoute()` 안에 넣지 마라** — 넣는 순간 푸시 트레이 탭까지
> 함께 막히는데, spec 은 **푸시는 무변경**이라고 명시했다. 조건은 **목록 화면 쪽**에 있어야 한다.

아래는 승격 전 진단이다(2026-09-01 초판):
...
### §7.2 탭 불가 상태 — **2026-09-01 2차 개정으로 목록이 확장됐다**

🔵 **푸시 트레이 탭은 무변경이다.** 4종 모두 지금 그대로 이동한다 — 아래는 **앱 안의 목록**에만
적용된다. 푸시는 방금 일어난 일을 알리는 것이라 대상이 살아 있고, 목록은 과거를 훑는 것이라 아니다.

| 경우 | 목록에서 | 근거 |
|---|---|---|
| **신청** — 챌린지가 대기 중 | ✅ 탭 → 홈 | 받은 도전장 섹션에 실물이 있다 |
| **신청** — 처리·만료됨 | ❌ | 🔴 홈의 그 자리가 **비워졌다.** 보내면 *"거기 뭔가 있다"* 고 말해놓고 아무것도 안 보여준다 |
| **증거 도착** | ✅ 항상 탭 → 상세 | 인증 사진은 **상세에만 있고 챌린지가 끝나도 사라지지 않는다** |
| **수락** | ❌ | 계약서는 남지만 홈·보관함에서도 닿는다 |
| **거절** | ❌ | 🔴 **대상 자체가 없다** — 계약서가 만들어지지 않는다 |
| 모르는 타입 (§3.2.2) | ❌ | 목적지를 모르는데 누르게 하면 아무 일도 안 일어나거나 엉뚱한 데로 간다 |
| `referenceId == null` | ❌ | 목적지를 특정할 수 없다. `PushEvent.from()` 이 이미 같은 판단을 한다(`challengeId?.let(::…)`) |

⚠️ **누를 수 없다는 것을 시각으로 표시하지 않는다** — 흐리게 처리하면 "고장난 알림"으로 보인다.
그냥 리플이 안 뜰 뿐이다(`onClick != null` 일 때만 `clickable`). 🔴 **이 문장은 2차 개정 후에도
그대로 유효하다** — 대신 **탭 가능한 쪽에 chevron 을 더한다**(§3.5). 낮추는 게 아니라 올리는 것이다.

🔴 **"대기 중" 은 앱이 판단하지 않는다.** 서버가 사실을 내리고 앱은 그걸 읽어 UI 정책을 편다
(spec 2차 개정 제약 1 — `type` 을 원문 String 으로 낸 것과 같은 결). **shape 은 backend 소유**이므로
이 문서는 필드명을 정하지 않는다 — 이 절의 명세는 *"탭 가능 여부가 주어졌을 때 어떻게 보이나"* 다.
...
| 상대 시각 · 부제 · 미지 타입 아이콘 · **chevron**(§3.5) | `onSurfaceVariant` | colors.md §1.2 |
| **날짜 그룹 헤더**(§3.6) | `onBackground` + `typography.bold14` | colors.md §1.2 · tokens.md §5.2 |
...
**타이포 슬롯도 신규 0건이다.** ✅ **2차 개정(§3.5 chevron · §3.6 날짜 그룹 헤더) 후에도 유지된다** —
chevron 은 [mypage §1.4.4](../mypage/design.md) 메뉴 카드와 같은 값이고, 헤더는 정본 섹션 헤더 문법
(`bold14`/`onBackground`)이다. 둘 다 이미 쓰이는 슬롯이다.
...
| ⑨ | 🔴 **탭 어포던스 chevron** — 정본 알림 카드에 chevron 이 **없다**(마이페이지 메뉴 카드에는 있다) | **추가**(§3.5). 정본은 6종이 전부 탭 가능한 mock 이라 구분할 필요가 없었다. 2차 개정으로 **비탭이 다수**가 되면서 필요해졌다. 값은 [mypage §1.4.4](../mypage/design.md) 와 동일 |
| ⑩ | 🔴 **날짜 그룹 헤더** — 정본에 **없다**(mock 6종이 헤더 없이 평평하다) | **추가**(§3.6). 정본의 **섹션 헤더 문법**(`index.tsx:223`)을 승계했고 신규 토큰 0건. `오늘`/`어제`/`M월 d일` 문구와 **건수 미표기**는 정본 근거가 없다 |
| ⑪ | **지난 신청 알림이 탭 불가로 바뀌는 것을 설명하지 않는다** | **설명 없음**(§3.5.3). chevron 부재가 신호 전부다. 문구를 붙이면 카드가 **챌린지의 현재 상태를 주장**하게 되는데 목록은 스냅샷이라 그 주장을 지킬 수 없다 |

---

## §11 pm-lead 백로그 요청 — ①~⑤ ✅ **처리 완료** (`0bd8b13`) · ⑥ 🟢 **신규 (2차 개정)**

> 🔴 **①~⑤ 는 열린 요청이 아니다.** pm-lead 가 처리했고 그 행들은 **무엇을 어떻게 처리했는지의
> 기록**이다. 남은 작업으로 읽지 마라.
> **⑥만 2차 개정에서 새로 나온 항목**이라 아직 등재 전이다.
...
| ⑥ | ⚠️ **앱에 날짜 방언이 둘이다** — `deadlineAbsoluteText`(`DeadlineAbsoluteText.kt:25,29,34`)는 `M/d`(슬래시), 보관함·이 화면은 `{M}월 {d}일`. **이 feature 가 만든 문제가 아니라 물려받은 것**이고, 여기서 `M/d` 로 갈아타면 보관함과 어긋난다 | 🟢 **백로그 후보.** 지금은 더 최근 확정인 `{M}월 {d}일` 쪽에 붙는다(§3.6.1). ⚠️ 시:분은 방언이 아니다 — `HH:mm` zero-pad 로 `deadlineAbsoluteText` 와 **이미 일치** |
...
| 2026-09-01 14:50:00 | 🔴 **2차 개정 (spec 2차 개정 수신) — 신설 2절 + 개정 4곳. 신규 토큰 0건 유지.** ① **§3.5 탭 어포던스 신설.** pm 이 짚은 충돌이 이 절의 출발점이다 — `"탭 비활성"` 에 §3.2.2 의 **모르는 타입**(무채색)이라는 뜻이 이미 있어서, 수락·거절을 같은 표현에 태우면 **정상 알림이 고장난 알림처럼 보인다.** 해법은 **두 축 분리**: 아이콘 색 = *"종류를 아는가"*(타입 축), chevron 유무 = *"갈 곳이 있는가"*(목적지 축). 축이 독립이라 안 겹친다. 🔴 **§7.2 를 뒤집지 않는다** — 비탭을 낮추지 않고 탭에 신호를 더하는 것이라 *"누를 수 없다는 걸 시각으로 표시하지 않는다"* 는 그대로 유효하다. chevron 은 `KeyboardArrowRight` 16dp/`onSurfaceVariant`([mypage §1.4.4](../mypage/design.md) 와 **같은 값**), 🔴 `Modifier.align(CenterVertically)`(Row 기본이 `Top` 이라 2줄에서 첫 줄에 붙으면 카드 전체의 기호가 한 줄만 가리키는 것처럼 보인다), **자리 예약 없음**. ② 🔴 **§3.5.3 — 시간이 지나 탭 불가로 바뀌는 것을 설명하지 않는다**(pm 이 명시로 물은 항목). 근거: **알림은 사건의 기록이지 대상의 현재 상태가 아니다** — `"처리됨"` 을 붙이면 카드가 챌린지의 현재 상태를 주장하는데 목록은 **스냅샷**이라 못 지킨다(다른 탭에서 수락하고 돌아오면 거짓). chevron 은 *"지금 누르면 갈 곳이 있나"* 라는 **화면 자신에 대한 사실**이라 낡아도 거짓말은 안 한다. 게다가 설명 문구는 반드시 *"그래서 어떻게 됐는데?"* 를 부르는데 이 화면은 못 답한다 → §3.2.2 의 *"의미를 모르는데 의미색을 주면 없는 뜻을 지어낸다"* 와 같은 자리. **톤 다운은 명시 기각**(충돌을 되살린다). ③ **§3.6 날짜 그룹핑 신설.** 🔴 핵심은 **§3.4 다섯 가지 중 날짜를 말하는 ④`어제`·⑤`M월 d일` 이 헤더로 승격**되고 시간을 말하는 ①~③ 이 행에 남는 것 — **그룹 = 달력 축, 행 = 그 날 안에서 언제**. `오늘` 그룹은 상대 시각, 나머지는 `HH:mm`. 행이 헤더를 절대 반복하지 않는다. ⚠️ 이것이 **§3.4 가지 ③의 24시간 경계를 무력화**한다고 정직하게 적었고(손실이 아니라 축 정리), 🔴 **그렇다고 §3.4 를 잘라내지 마라** — ③·④ 순서 논증을 고정한 반례 테스트가 함께 죽고 그게 그 함수의 유일한 방어다. 헤더는 정본 섹션 헤더 문법(`index.tsx:223` `text-sm font-bold`) 승계 → `bold14`/`onBackground`, **건수 미표기**(지난 날짜의 알림 수는 아무 행동도 바꾸지 않는다 — 홈의 "받은 도전장 3건" 은 처리할 대상이라 다르다), **sticky 아님**(앱 선례 0건 + 알림 그룹은 짧아 sticky 의 값이 안 나온다 + 고정 크롬 3층 회피). 🔴 **§3.6.3 커서 경계** — 그룹을 페이지 단위로 만들면 한 날짜가 경계에 걸릴 때 **같은 헤더가 두 번** 찍힌다. 앱이 **누적 목록 전체를 기준으로** 계산하고, **계약에 그룹 필드를 넣지 않는다**(서버는 페이지 단위로만 아는 주체라 원천적으로 못 만든다). 자정 갱신은 요구하지 않음(§3.4 의 "초 단위 갱신 안 함" 과 같은 결 — 진입 시각 스냅샷). ④ **§7.2 개정** — 탭 불가 목록을 상태 기반 7행으로 확장. 🔵 **푸시 트레이 탭 무변경**을 절 머리에 박았다. 🔴 **"대기 중" 은 앱이 판단하지 않는다** — shape 은 backend 소유라 필드명을 정하지 않고, 이 문서는 *"탭 가능 여부가 주어졌을 때 어떻게 보이나"* 만 명세한다. ⑤ **§7.1 해소 표기** — `toRoute()` 가 `:core:push/PushEventRoute.kt:10` 으로 승격·public 화되어(T-M2 완료) 초판 진단이 해소됐다. 🔴 **사본을 만들 수 있던 구조 자체가 없어진 것**이지 "안 만들기로 한" 게 아니다. ⚠️ 2차 개정 주의 하나 추가 — **탭 가능 조건을 `toRoute()` 안에 넣지 마라**(넣으면 푸시 트레이까지 막히는데 spec 은 푸시 무변경이다). 조건은 목록 화면 쪽. ⑥ §1 골격에 헤더·푸터 반영, §9 토큰표 2행 추가(**신규 0건 유지** — 둘 다 기존 슬롯), §10 확인 대상 ⑨⑩⑪ 신설. | design-bridge (noti-design) |
| 2026-09-01 15:05:00 | **2차 개정 보강 3건 (noti-mobile 질의·실측). 판정 변경 0건.** ① 🔴 **`HH:mm` 이 새 방언인지 실측했다 — 아니다.** noti-mobile 이 *"앱에 시:분 선례가 있는지 확인 못 했다"* 고 물었고, `DeadlineAbsoluteText.kt:32-34` 가 이미 `padStart(2,'0')` 로 zero-pad 된 시:분을 만든다. → **zero-pad 를 따른다**(`09:05`, `9:05` 아님). ⚠️ 다만 **날짜 부분은 앱에 방언이 둘**이다(`deadlineAbsoluteText` 는 `M/d`, 보관함·이 화면은 `{M}월 {d}일`) — **이 feature 가 만든 게 아니라 물려받은 것**이라 더 최근 확정인 `{M}월 {d}일` 에 붙이고 §11-⑥ 에 등재했다. ② 🔴 **자정 경계 — 타이머 만들지 않는다.** noti-mobile 이 *"헤더는 `오늘` 이라는 단어라 어긋남이 더 눈에 띈다"* 고 정확히 짚었고 그 지적은 맞다. 그럼에도 유지하는 근거를 강화했다 — **자정에 갱신하면 목록이 눈앞에서 재구성된다**(`오늘` 그룹 전체가 `어제` 로 내려가고 읽던 자리가 밀린다). **낡은 헤더는 수동적 부정확이고 자정 재구성은 능동적 방해**라 후자가 더 나쁘다. 실패 조건도 *"자정 순간에 목록을 띄워 놓고 보고 있을 것"* 으로 좁다. 요구는 결과 하나(**화면을 다시 열면 맞을 것**)로 냈고 수단은 mobile 판단(§4.5.1 과 같은 형태). ⚠️ **`now` 가 ViewModel 생애당 1회면 이 요구가 성립하지 않는다**(구독당 1회여야 한다)는 확인 요청을 붙였다. ③ **§3.5.3 에 확인 답 추가** — noti-mobile 이 *"지난 신청은 아이콘·색이 똑같고 화살표만 사라진다. UiState 축을 하나 더 만들까"* 물었고 **필요 없다**로 답했다. 🔴 **그 카드가 알리는 사건은 변하지 않았다** — *"민수가 신청했다"* 는 어제도 오늘도 참이고 아이콘·색은 그 **사건**을 그린다. 변한 건 목적지의 유무 하나뿐이라 목적지 축의 신호 하나만 바뀌는 게 정확하다. **카드가 사건을 다르게 말하기 시작하면 그때부터 거짓이다.** ④ 부수: §11 머리가 *"전부 처리됨"* 이라 신규 ⑥ 과 모순돼 `①~⑤ 완료 · ⑥ 신규` 로 정정. | design-bridge (noti-design) |
| 2026-09-01 15:25:00 | 🔴 **§3.6.3 이 사실이 아닌 것을 사실로 적고 있었다 — 정정 + §3.6.3.1 신설.** 초판이 *"화면을 벗어났다 돌아오면 재구독이 일어나며 `now` 를 다시 잡는다"* 고 **단정문으로** 적었는데 **거짓이었다.** 내가 붙여 둔 확인 요청(*"ViewModel 생애당 1회인가 구독당 1회인가"*)에 noti-mobile 이 실측으로 답해 드러났다 — `NotificationViewModel.kt:60,173` 의 `now` 는 **생애당 1회**였고 `init { loadFirstPage(); markAllAsRead() }` 도 마찬가지라 **재진입해도 아무것도 다시 안 받았다.** 알림 화면은 push 로 열리는 하위 화면이라 상세를 다녀와도 백스택에 남아 ViewModel 이 살고, 따라서 **자정 한 번이 아니라 앱을 켜둔 내내** 틀린다(헤더뿐 아니라 행의 상대시각/`HH:mm` 갈림까지). 🔴 **왜 초판에 안 보였나** — 그룹핑 전에는 낡음이 *"3시간 전이 5시간 전이 안 됨"* 이라는 **정도 문제**였는데, **§3.6 이 그것을 `오늘`/`어제` 라는 단정문으로 승격**시켰다. **내 개정이 만든 새 조건인데 §3.4 의 스냅샷 정책을 그대로 승계하면 되는 줄 알았던 것**이 오류의 정체다. 본문의 단정을 **요구(§3.6.3.1)로 강등**하고 실측·해소를 기록했다. 🔵 **backend 가 독립적으로 같은 자리를 짚었다** — `referencePending` 을 캐시하면 **오래된 `true` 로 만료된 신청의 탭을 허용**한다. **두 요구의 해법이 하나**이고(재진입마다 다시 받으면 `now` 도 `referencePending` 도 새로워진다) 원인이 **같은 캐시**라는 점을 등재. 해소는 mobile 확정안(로딩을 `init` 에서 재진입 자리로 이동, §4.5.1 이 기대는 것과 같은 구조라 새 메커니즘 아님). ⚠️ 부수 효과 **재진입마다 `read-all` 재실행은 §4.3 규약에 맞다**고 판정 — 첫 진입에만 처리하면 **사용자가 목록을 보고 있는데 뱃지가 켜져 있는** 상태가 된다. 불변식은 *"쪽마다 다시 돌지 않는다"* 이지 *"생애당 1회"* 가 아니다. 부수: §3.6.1 에 `deadlineAbsoluteText` **재사용 불가**(padding 이 `"M/d HH:mm"` 문자열에 박혀 있고 `ChallengeDateTimeFormats` 는 wire 전용) → 신규 `toClockTimeText()` + **관용구만 승계** 기록. 🔴 **함수가 둘인 것과 표기가 둘인 것은 다르다.** | design-bridge (noti-design) |
| 2026-09-01 15:45:00 | **§3.6.3.2 신설 — 재진입 재로딩의 대가를 명시 선택했다 (pm 요구).** pm 이 *"셋 중 하나로 명시하라"* 며 ① 감수+명세 ② 1쪽만 재조회+나머지 유지 ③ 다른 안을 제시했다. → 🔴 **①을 택했다.** **②를 기각한 근거가 결정적이다** — *"새 1쪽 + 낡은 2~n쪽"* 을 이어붙이면 자리를 비운 사이 도착한 알림만큼 **옛 1쪽 꼬리가 밀려나는데 낡은 2쪽이 그걸 그대로 들고 있어 같은 항목이 두 번 나온다.** [ADR-0012 §근거 1](../../decisions/0012-list-pagination-cursor.md) 이 offset 을 기각한 사유가 정확히 그것(*"머리에 새 행이 들어오는 동안 페이지가 밀려 2페이지에 1페이지 항목이 다시 나온다"*)이고, 같은 ADR 이 **알림을 그 실패가 `상시` 조건인 도메인으로 지목**했다. 🔴 **커서를 고른 이유가 이 중복인데 부분 재로딩이 커서 위에서 그 중복을 다시 만든다.** 부차 근거 — 신선도가 **부분적으로만** 참이 되고(1쪽은 새 `now`·`referencePending`, 2~n쪽은 낡음) **화면에서 둘이 구분되지 않아** 전부 낡은 것보다 나쁘다. ①의 대가가 작은 이유도 적었다: 탭 대상(증거 도착)은 *"상대가 방금 인증했다"* 라 **신선도 쪽에 쏠리고**, 이 화면의 값은 spec 정의대로 *"푸시를 놓치면 그걸로 끝"* 을 메우는 것인데 **놓친 푸시는 목록 머리에 있다.** 🔴 **"재진입하면 목록이 처음으로 돌아간다"를 명세로 못박고 "버그가 아니다, 고치지 마라"를 붙였다** — pm 지적대로 안 적으면 다음 사람이 결함으로 보고 고치는데 고치는 순간 §3.6.3.1 의 캐시 문제가 통째로 되살아난다. **되돌아볼 조건**(깊은 쪽 탭이 실제로 관측되면)과 그때도 **부분 재로딩으로 가지 말 것**을 함께 남겼다. | design-bridge (noti-design) |
| 2026-09-01 16:00:00 | **§3.6.3.1 에 2단 검사 추가 (pm-lead 지시).** 이 절의 오류에서 재사용 가능한 검사를 뽑아 남겼다. **두 문장을 함께 둔다** — ① **질문**(내 원문): *"이 정책이 견디던 하중이 새 자리에서도 같은가?"* — 승계는 정책만 옮기고 하중은 안 본다 ② 🔴 **판정 기준**(pm-lead): *"그 정책이 견디던 것이 **오차**인가 **문장의 참·거짓**인가?"* — `"3시간 전"`→`"5시간 전"` 은 오차라 스냅샷이 견디고, `오늘` 이 어제를 가리키는 것은 거짓이라 안 견딘다. pm-lead 판단대로 **원문을 지우지 않았다** — ①은 질문을 던지는 형태고 ②는 판정하는 형태라 *"하중을 보라"* → *"오차인가 참·거짓인가로 갈라라"* 순서로 다른 일을 한다. **명세 변경 0건** — 재발 방지 기록이다. | design-bridge (noti-design) |
