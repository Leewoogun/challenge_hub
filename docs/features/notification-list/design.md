...
항목은 chart 5색), mypage §5.1 은 확인 대상이 아니라 관찰 기록이다. **이 절이 그 관찰에 대한 회신**이고,
pm-lead 가 mypage §5.1 에 역참조를 남길지 판단한다(§11-③).
...
| # | 항목 |
|---|---|
| ① | ~~**`hasUnreadNotification = false` 하드코딩**~~ ✅ **소멸 (2026-09-01).** 읽음 처리 (B) 채택으로 뱃지가 이번에 점등된다(§4.5). 등재할 죽은 UI 가 없다 |
| ①' | ⚠️ **뱃지 위치 육안 검증** — 정본은 버튼 기준 6px(`top-1.5 right-1.5`), 구현은 10dp(`HomeTopBar.kt:80`). 40dp 버튼에 24dp 아이콘이라 여백이 다르다. **이번 feature 에서 점이 처음 켜지므로** 실물을 보고 판단. 지금은 안 고친다(§4.5) |
| ② | **`EmptyStateCard` 의 CTA 필수 제약** — 이번이 CTA 없는 빈 상태의 **두 번째** 사례다([mypage §2.5.1a](../mypage/design.md) 가 첫 번째). 세 번째가 나오면 `onClickCta` optional 화를 판단. **지금은 하지 않는다** |
| ③ | **[mypage §5.1](../mypage/design.md) 에 역참조** — *"`success` 를 쓰는 자리가 사라졌다"* 는 관찰이 이 화면으로 해소됐다(§9.1). 문서 간 연결만 |
| ④ | **repos.json `mobile.modules` 가 낡았다** — 실측(`settings.gradle.kts`) 대비 `:core:push` · `:core:permission` 2개가 목록에 없다. 이 feature 가 `:core:push` 를 참조하므로(§7.1) 등재 필요 |
| ⑤ | ⚠️ **[home-feed/design.md:34](../home-feed/design.md) 의 `"2.dp dot"`** — Tailwind `w-2` 는 8px 이고(`index.tsx:130` `w-2 h-2 bg-destructive`) 구현도 8dp 다(`HomeTopBar.kt:81`). **구현이 맞고 문서가 틀렸다.** 출시된 화면이라 코드는 손대지 않는다 |
