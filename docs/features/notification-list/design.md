...
| **`HomeScreen.kt:61`** | 🔴 **`hasUnreadNotification = false` 하드코딩** — `HomeScreen(uiState: HomeUiState.Data, …)` 안. 데이터가 있는데 안 쓴다 |
| `HomeScreen.kt:125` | ⚠️ **여기도 `false` 지만 성격이 다르다 — 고치지 마라.** §4.5.2 |
...
#### §4.5.2 🔴 로딩 중에는 점을 그리지 않는다 — `HomeScreen.kt:125` 를 고치지 마라

**초판이 `:61` 과 `:125` 를 *"호출부 전부"* 로 묶어 적었다. 그게 오독을 부른다**(noti-mobile 지적,
2026-09-01). 두 호출부는 성격이 다르고, **바꾸는 것은 `:61` 하나뿐이다.**

| 위치 | 소속 | 시그니처 | 판정 |
|---|---|---|---|
| `:61` | `HomeScreen` | `(uiState: HomeUiState.Data, …)` — **데이터 있음** | ✅ **실제 값을 흘린다** |
| `:125` | **`HomeLoadingScreen`** | `(onBellClick, onFabClick)` — 🔴 **콜백 둘뿐. 데이터가 없다** | 🔴 **`false` 그대로 둔다** |

**`:125` 의 `false` 는 하드코딩이 아니라 *"아직 모른다"* 의 정직한 표현이다.** 조회가 끝나지 않아
안 읽은 개수를 모르는 시점이라, 여기서 점을 켜려면 **값을 지어내야 한다.**

🔴 **그리고 켰다가 응답이 0 이면 점이 깜빡 켜졌다 꺼진다** — §4.5.1 이 막으려는 *"읽었는데 왜 아직
있지"* 와 **같은 종류의 불신을 로딩 구간에서 만든다.** 방향만 반대일 뿐 원인이 같다(뱃지가 사실이
아닌 순간을 갖는 것). §4.5.1 과 이 절은 **한 규칙의 양끝**이다 — **뱃지는 모를 때 켜지지 않고,
알고 난 뒤엔 늦게 꺼지지 않는다.**
...
- 목록의 **마지막 item** 으로 `CircularProgressIndicator`(20dp) + 상하 16dp 패딩, 가운데 정렬,
  `colorScheme.primary`.
- **`strokeWidth = 2.dp`** — 🔴 Material 기본 4dp 는 20dp 원에서 도넛처럼 두껍다. 첫 조회의 전체 화면
  인디케이터와 달리 이건 **카드 사이에 끼는 작은 표식**이라 굵기가 같으면 안 된다.
  (구현이 먼저 잡은 값이고 판정으로 승인했다 — `NotificationListFooter.kt:46`.)
...
##### `"다시 시도"` 행의 시각 규격 (2026-09-01 확정 — noti-mobile 질의)

초판에 **자리만 있고 형태가 없었다.** 구현이 먼저 잡았고, 판정으로 승인한다:

| 요소 | 값 |
|---|---|
| 배치 | `Row`, 가운데 정렬, 전폭 `clickable`, 상하 **16dp** |
| 아이콘 | `Icons.Filled.Refresh` **16dp**, `colorScheme.primary` |
| 간격 | 아이콘–텍스트 **6dp** |
| 라벨 | `"다시 시도"`, `typography.medium14`, `colorScheme.primary` |
| 카드 크롬 | 🔴 **없음** |

- **카드 크롬을 주지 않는 이유**: 이건 목록의 **항목이 아니라 목록의 꼬리**다. `surface` + `outline`
  을 두르면 알림 카드 한 장으로 보이고, 사용자가 **알림을 눌렀다고 생각한다.**
- **아이콘을 넣는 이유**: `ArchiveFailureScreen` 이 같은 라벨에 `ctaIcon = Icons.Filled.Refresh` 를
  쓴다(`ArchiveScreen.kt:208`). **앱에서 `"다시 시도"` 는 이미 `Refresh` 를 달고 다닌다** — 여기서만
  텍스트 단독이면 같은 행동이 두 어휘를 갖는다.
- 🔴 **로딩과 재시도가 같은 높이를 써야 한다.** 상하 패딩을 **공유 상수 하나**로 두고 둘 다 참조한다
  (`NotificationListFooter.kt:100` `FooterVerticalPadding`). 높이가 다르면 교체되는 순간 목록이 튄다 —
  `ArchiveFailureScreen` 이 같은 이유로 빈 상태와 자리를 맞춘 것과 같다.
- ⚠️ **실패 사유 텍스트를 인라인으로 넣지 않는다.** 그건 스낵바가 말한다. 두 자리에 같은 말을 쓰면
  꼬리가 커지고, `"다시 시도"` 라는 라벨 자체가 이미 *"실패했다"* 를 함의한다.
...
| 2026-09-01 14:20:00 | **구현 중 실측 반영 2건 (noti-mobile 질의·지적). 판정 변경 0건, 누락 보완 2건.** ① 🔴 **§4.1 의 *"호출부 전부"* 가 오독을 부르는 기재였다 — 정정하고 §4.5.2 신설.** `HomeScreen.kt:61`(`HomeScreen(uiState: HomeUiState.Data, …)`)과 `:125`(**`HomeLoadingScreen(onBellClick, onFabClick)`** — 콜백 둘뿐, 데이터 없음)를 직접 대조했고 **noti-mobile 지적이 맞다.** `:125` 의 `false` 는 하드코딩이 아니라 *"아직 모른다"* 의 정직한 표현이라 **그대로 둔다** — 조회 전에 점을 켜려면 값을 지어내야 하고, 켰다가 응답이 0 이면 **점이 깜빡 켜졌다 꺼진다.** 🔴 그건 §4.5.1 이 막으려는 불신과 **원인이 같다**(뱃지가 사실이 아닌 순간을 갖는 것). 두 절을 한 규칙의 양끝으로 묶었다 — **뱃지는 모를 때 켜지지 않고, 알고 난 뒤엔 늦게 꺼지지 않는다.** ② **§6.3 에 시각 규격 2건 보완** — 초판이 `"다시 시도"` 행의 **자리만 정하고 형태를 안 정했고**(noti-mobile 질의), 인디케이터도 `strokeWidth` 가 없었다. 구현이 먼저 잡은 값을 확인하고 승인 + 사유를 붙였다: `Refresh` 16dp + `medium14`/`primary` + **카드 크롬 없음**(항목이 아니라 목록의 꼬리 — 크롬을 주면 알림을 누른 걸로 오해한다) · 아이콘 존치(`ArchiveScreen.kt:208` 이 같은 라벨에 이미 `Refresh` 를 쓴다 — 같은 행동이 두 어휘를 가지면 안 된다) · **상하 패딩 공유 상수**(높이가 다르면 교체 순간 목록이 튄다) · `strokeWidth = 2.dp`(기본 4dp 는 20dp 원에서 도넛처럼 두껍다). 실패 사유 인라인 텍스트는 **넣지 않는다** — 스낵바가 말하고, `"다시 시도"` 가 이미 실패를 함의한다. | design-bridge (noti-design) |
