...
> 🔵 **이 절은 [ADR-0012](../../decisions/0012-list-pagination-cursor.md) 로 승격됐다**(2026-09-01).
> **프로젝트 규약은 ADR 이 정본**이고 이 절은 알림에 적용한 구체 근거로 남는다 — 다른 목록에
> 적용할 때는 ADR 을 봐라. CLAUDE.md 규칙 줄도 같은 결정을 가리킨다.
...
**기존 6종을 지금 따라 고치지 않는다** — 보관함은 월이 이미 경계이고, 홈 진행중·받은 도전장은
설계상 한정이며, 친구·받은 요청·랭킹은 백로그가 말한 *"수백 건 도달"* 신호가 아직 없다.
다만 **다음에 도입하는 쪽은 이 shape 을 따른다**: `?cursor=&size=` +
`data.nextCursor: <키>?`, 정렬·커서 키 동일, 불리언 금지.

> 🔴 **2026-09-01 정정 — 이 문단이 처음에 "나머지 4종(친구 목록·받은 요청·랭킹·보관함)"이라고
> 적었다. 오답이다.** `GET /challenges/active` 와 `GET /challenges/received` 를 빠뜨렸고, 실측
> 기존 목록 엔드포인트는 **6개**다([ADR-0012 §맥락](../../decisions/0012-list-pagination-cursor.md)).
> ⚠️ **이 숫자가 CLAUDE.md 규칙 줄과 백로그로 그대로 퍼졌다** — 계약에 적은 미검증 숫자가
> 어떻게 번지는지의 사례라 지우지 않고 남긴다. 사유별 분류는 ADR §적용 범위가 정본이다.
...
> ⚠️ **오늘 서버는 enum 밖 값을 내보내지 못한다** — `NotificationEntity.toDomain()` 이 모르는
> `type` 을 `CHALLENGE_REQUEST` 로 떨어뜨린다(backend-report 미해결 1, 실측 재현됨).
> **새 타입이 추가되는 예정 경로에서는 enum 에도 함께 들어가므로 이 절의 논증이 그대로
> 성립한다** — 대기 중인 4종(`REMIND`/`RESULT`/`TAUNT`/`FRIEND_REQUEST`)은 **이미 enum 에 있어
> 원문 그대로 도착한다**(실측: `FRIEND_REQUEST` round-trip 확인). 강등에 걸리는 것은 **롤링
> 배포·수동 삽입뿐**이다.
> 🔴 **그러므로 앱의 폴백을 "쓸모없는 방어"로 읽지 마라.** 앱이 방어해야 할 실제 케이스는
> 전부 앱까지 도달한다. 이 문장을 근거로 design §3.2 의 미지 타입 폴백(무채색 아이콘 + 비탭)을
> 걷어내면, 트리거가 왔을 때 막을 것이 하나도 없다.
...
### 🔴 계약 조항 — **앱은 미지 타입을 무시하지 않고 표시한다**

spec 수용 기준의 *"모르는 타입이 와도 목록이 깨지지 않아야 한다"* 는 **중립적으로 degrade
하라**는 뜻이지 **숨기라**는 뜻도 **아는 타입인 척 하라**는 뜻도 아니다. 계약으로 못박는다:

| | 규약 |
|---|---|
| 목록에서 숨기기 | 🔴 **하지 않는다.** 숨기면 **푸시로 본 알림이 목록에 없고**, 안 읽은 개수와 목록 길이가 어긋난다 (design §3.2.2) |
| 표시 | 무채색 아이콘(`onSurfaceVariant`) — 의미를 모르는데 의미색을 주면 **없는 뜻을 지어낸다** |
| 문구 | 서버 `body` 를 그대로 그린다 — **모르는 타입도 보여줄 문구는 있다**(박제분) |
| 탭 | **비활성.** `referenceId == null` 도 같은 처리 |

🔵 **`body` 를 박제로 저장한 결정(push-fcm 오픈이슈 2)이 여기서 값을 한다** — 앱이 타입별로
문구를 조립하는 구조였다면 모르는 타입은 **그릴 문구조차 없었다.**

### 커서는 **소비되지 않는다** — 재시도는 같은 값으로 다시 부르면 된다

design §6.3 이 물은 것에 답한다. `cursor` 는 서버가 상태로 들고 있는 토큰이 아니라 **`id` 비교
값**이다(`WHERE id < :cursor`). 그래서:

- **같은 커서를 몇 번을 보내도 같은 페이지**가 온다. 부분 실패 후 *"다시 시도"* 를 **같은
  커서로** 부르는 설계가 그대로 성립한다.
- **없는 `id` 를 커서로 보내도 에러가 아니다** — 그보다 작은 것들이 온다(실측: `cursor=99999`
  → 200). 그 사이에 알림이 삭제돼도 목록이 이어진다.
- 만료도 없다.
