...
- **완료일**: 2026-09-02
- **상태**: **completed** (2026-09-05 종료) — 전 범위 구현·검증 완료 + **양 레포 커밋**(app `2a650ba` / server `e2f57f5`) + **T-I1 실기 3/4 확인**(사용자 육안). 잔여 1건은 §미해결 이슈 — 비블로커
...
- [x] ✅ **T-I1 실기 왕복 — 3/4 확인** (2026-09-05, 사용자 육안). ① 내가 교체 → 같은 화면·재진입·앱
      재실행 ② 🔴 **상대가 교체 → 내 화면 재진입** ③ 교체해도 `OPPONENT_VERIFIED` 알림이 추가로 오지
      않음. 🔑 **이 feature 의 실제 난점(캐시)은 ①②로 닫혔다** — 단위 테스트가 원리적으로 못 잡는
      항목이 정확히 그 둘이었고, 둘 다 사람 눈으로 통과했다
- [ ] 🟢 **T-I1 ④ 서버 실디스크 파일 삭제만 실기 미확인** — **블로커 아니다.** 근거 둘:
      ⓐ `FakePhotoStorage` 로 **호출 여부·삭제 key·커밋 후 순서까지 단언**돼 있다(관련 테스트 4건:
      `재제출하면 이전 사진 파일이 삭제된다` / `첫 제출은 아무것도 삭제하지 않는다` /
      `이전 파일 삭제가 실패해도 교체는 성공한다` / `key 가 없는 VERIFIED row 에 제출해도 삭제를
      시도하지 않는다`) ⓑ `PhotoStorage.delete` 는 **이 feature 가 만든 코드가 아니다** — 회원탈퇴가
      이미 쓰던 기존 구현이다. 남은 미검증은 *"그 기존 구현이 실파일을 지우는가"* 하나이며, 실패해도
      **고아 파일이 남을 뿐 교체는 성공**한다(ADR-0011 보관 기간 정책 소관, spec §6 이 이미 명시)
- [x] ✅ **양 레포 커밋 완료** (2026-09-05) — app `2a650ba` / server `e2f57f5`. 🔵 **둘 다 미푸시**
