...
- [ ] 🔴 **교체 후 화면에 새 사진이 보인다** — 같은 화면 / 재진입 / 앱 재실행 3경로 모두.
      🔴 **상대가 교체한 경우도 포함**(내 교체는 로컬 상태가 가려주지만 상대 것은 순수 캐시 문제다).
      단위 테스트로 못 잡으므로 **T-I1 실기에서 사람이 눈으로 확인**한다
...
**T-M4. 🔴 Coil 캐시 무효화** — *(2026-09-02 추가. 착수 후 발견돼 원안에 없던 태스크다)*

**이게 없으면 이 feature 는 사용자 눈에 아무 일도 일어나지 않는다.** 사진 경로는 교체돼도
그대로이고(`/api/v1/challenges/{id}/photos/{party}`), Coil 이 URL 을 캐시 키로 쓴다
(`VerificationPhoto.kt:103` `model = url`). 즉 **다시 찍어도 옛 사진이 그대로 뜬다.**

🔴 **이 실패는 조용하다** — 에러도 로그도 없고 앱은 정상적으로 캐시를 쓴 것뿐이다. 단위
테스트로 못 잡는다(런타임 캐시 동작). **실기에서만 드러난다.**

- **방식**: `ImageRequest` 의 `memoryCacheKey` / `diskCacheKey` = `photoUrl + verifiedAt`.
  URL 은 안 바꾼다 → 계약 §4 `photoUrl` 모양 무변경, 서버 작업 0
- **왜 되는가**: 교체 시 서버가 `verified_at` 을 갱신하므로(T-B1 ④) 키가 반드시 바뀐다.
  **내 교체·상대 교체를 구분할 필요가 없다** — §4 재조회만으로 신선도가 따라온다
- 🔵 **서버 ETag/304 는 앱이 타지 않는다.** Coil 3 기본 `CacheStrategy` 가 HTTP 캐시 헤더를
  무시하기 때문이다. 헤더 자체는 올바르고 다른 클라이언트·향후 이사에 유효하므로 유지한다
- ⚠️ **인지된 한계**: `verifiedAt` 이 초 단위(ADR-0010)라 **같은 초에 두 번 교체하면** 키가
  겹친다. 촬영→확인→제출→업로드 왕복이 1초에 두 번 일어날 수 없어 도달 불가로 본다.
  🔴 **여기에 방어 로직을 넣지 마라** — 관측된 버그가 아니라 가정이다. 한계만 리포트에 남긴다
- 🔴 **`verifiedAt` 이 null 인 `PENDING` 상대**의 키 처리와, `PENDING → VERIFIED` 전이 시
  사진이 제대로 나타나는지 확인할 것

**T-M3. 테스트 + 빌드 검증** — 결과는 **숫자로**

🔴 **T-M4 는 "구현했다" 로 닫지 마라.** 보고 전에 `grep -rn "memoryCacheKey\|diskCacheKey"
--include="*.kt"` 를 직접 돌려 0건이 아님을 확인하고 **그 명령과 결과를 리포트에 적어라.**
이 항목은 2026-09-02 에 pm-lead 가 **승인을 완료로 잘못 기록**한 이력이 있다(backend-dev 적발).
