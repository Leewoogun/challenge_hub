...
# 🔴 3차 개정 — 월 전환 깜빡임 제거 (2026-09-01)

사용자 실기 지시: *"달 넘길 때마다 화면이 깜빡인다. 데이터를 읽어온 뒤에 바꿔서 깜빡임을 없애라."*
**계약 변경 0**, 코드 변경은 `ArchiveViewModel` + 그 테스트 **2파일**뿐이다.

## 🔴 규명 — 깜빡임은 스피너가 아니라 `Blank` 상태 자체였다

`monthFlow` 가 탭 즉시 **정지 없이** `(새 달, Blank)` 를 보내고 있었다. 화면은
**`Data(8월) → Blank(빈 화면) → Data(7월)`** 로 갔다.

**로컬 응답 실측 3~49ms** (`:8080` 3회). 🔴 **300ms 임계에 한참 못 미쳐 스피너는 실기에서 한 번도
뜨지 않았다** — 지연 스피너(§2.5.4.3-③)는 **의도대로 작동했고**, 사용자가 본 것은 그 사이의 **빈 화면
한 순간**이었다. **즉 스피너 지연값을 아무리 조정해도 고쳐지지 않는다.**

⚠️ 바의 건수도 같이 빈다는 것을 함께 찾았다 — `countText` 가 `Data` 가 아니면 `""` 라
탭마다 `"3건" → "" → "2건"`. design 이 바의 **기하**는 고정했지만 **내용이 비는 것**은 그 규칙 밖이었다.

## 🔴 핵심 — 깜빡임과 탭 피드백이 같은 메커니즘이었다

`Blank` 즉시 emit 은 *"라벨이 탭 즉시 바뀐다"*(§2.5.4.3-①)와 *"제목의 월이 탭 결과를
확인해 준다"*(§2.5.1a-①)가 **함께 서 있던 자리**다. 없애면 피드백도 없어진다.

그리고 `flatMapLatest` 와 맞물리면 더 나쁘다 — 전환을 응답까지 미루면 **연타 5번에 화면이 5번 다
안 움직이고 그러다 한 번에 5개월 점프**한다(중간 요청이 전부 취소되므로). 사용자가 확정한
*"빈 달을 하나씩 지나간다"* 가 화면에서 사라진다.

→ **처방(최종): 월 전환에서 화면을 비우는 경로를 통째로 없앤다.**

| 구간 | 화면 |
|---|---|
| 탭 ~ 응답 | 🔴 **이전 달을 온전히 유지** — 라벨·건수·카드 전부 그대로. **아무것도 emit 하지 않는다** |
| 300ms 초과 | 🔴 **건수 슬롯만 `"불러오는 중"`.** 라벨·목록은 **여전히 이전 달** |
| 응답 도착 | 라벨·건수·목록 **한 번에 교체** |
| 🔴 전환 실패 | **이전 달 유지 + 스낵바.** 실패 카드로 덮지 않는다 |
| 첫 로딩 | 기존대로 즉시 스피너, 실패 시 실패 카드 |

⚠️ **중간판(100ms 붙들다 `Blank`)을 거쳐 여기 왔다** — 그 안은 **느린 응답에서 빈 화면을 그대로
남겼다.** 지금은 `ArchiveContent.Blank` 가 **월 전환에서 도달 불가능**해졌다(첫 로딩 전용).

🔴 **로딩 힌트가 건수 슬롯을 쓰는 것이 이 설계의 요령이다** — 바가 *"모르는 구간에도 슬롯 유지"* 로
**이미 예약해 둔 자리**라 **새 자리를 만들지 않아 기하가 안 변한다.** 내가 찾아낸 그 공백이
여기서 값을 한다. `"불러오는 중"` 은 **건수를 주장하지 않으므로** 이전 달 라벨과 어긋나지 않는다.

🔴 **옛 짝 유지는 불변식 위반이 아니다** — 금지된 것은 **혼합**(`7월 라벨 + 8월 카드`)이고,
옛 짝은 **이미 참이었던 짝**이라 두 달을 말하지 않는다. 헤더·본문을 한 값으로 emit 하는 구조가
그대로라 혼합은 여전히 표현조차 되지 않는다. **이 변경이 싼 이유이기도 하다.**

## 🔴 지시 하나를 정정하고 구현했다 — 건수 유지

pm-lead 지시가 *"전환 중에는 건수의 옛 값을 유지한다"* 였는데, **라벨이 바뀐 뒤에 옛 건수를 끌고 가면
`(7월 라벨 + 8월 건수)`** 가 되어 **금지된 혼합 그 자체**다. 실제로는:
- **빠른 경로**: 전환 지연으로 옛 짝이 통째로 유지되므로 **건수도 코드 변경 없이 안 빈다** ✅
- **느린 경로**: 라벨이 이미 새 달이므로 **건수는 비운 채 둔다** — 그 달의 건수를 아직 모르는 것이 사실이다

design 에 이 정정을 통지했다.

## 🔴 부수 발견 — 40dp 배경이 Compose 기본 ripple 을 가린다 (소스 실측)

탭 피드백을 기본 ripple 로 낼 수 있는지 확인하려고 Material3 소스를 열었다.
`IconButtonImpl`(`material3-android-1.4.0-sources`, `IconButton.kt:175-188`)이
**`indication = ripple()` 을 Box 자신에 걸고 `content()` 를 그 위에 그린다.**

→ 2차 개정에서 넣은 **40dp 불투명 `secondary` 배경이 그 ripple 을 덮는다.** 컨테이너 크기가 40dp 라
겹침이 사실상 100% 다. 🔴 **결과가 고약하다 — 배경이 있는 *활성* 화살표에서 눌림 피드백이 가장 약해진다.**

⚠️ **2차 개정이 틀린 게 아니다** — 컨테이너 축을 만든 판단은 맞고 비활성 구분도 그 덕에 명확해졌다.
다만 그 배경이 **눌림 신호를 덮는 부작용**이 있었고, 이번에 눌림 피드백이 **주역**이 되면서 드러났다.
→ **눌림 시각 값을 design 에 요청했다**(state layer 를 배경 **위**에 얹는 (C) 안 권고).
⚠️ 그 값이 오기 전까지 **100ms 안의 연타는 화면 반응이 없다.**

## 테스트 (3차) — 실측 XML

**전체 Android 571 / iOS 486, `failures=0 errors=0`** (전 모듈 XML 전수). 기준선 565 / 480 →
**+6 / +6** (`:feature:mypage` 51 → **57**, 양 타깃 동일). **회귀 0.**
⚠️ gradle 종료 코드 1은 **선재 실패 4건**뿐이고 1·2차 마감 때와 같은 집합이다.

- 🔴 `응답이 빠르면 중간 상태 없이 새 달로 바뀐다` — **깜빡임 제거의 회귀 테스트.**
  `Blank` 도 `Loading` 도 방출되지 않고 곧바로 새 달 `Data` 가 온다
- 🔴 `전환 지연 동안에는 옛 달이 통째로 유지된다` — **기존 테스트를 재작성한 것.**
  옛 이름은 `월을 바꾸면 이전 달 카드가 남지 않는다` 였는데, 지켜야 할 불변식이
  *"즉시 비운다"* 에서 **"라벨과 본문이 언제나 같은 달"** 로 바뀌어 **단언이 틀린 것이 됐다.**
  이제 전환 지연 동안 **라벨·본문이 둘 다 옛 달**임을 단언한다
- `응답이 늦으면 백 밀리초 뒤에 본문이 비워진다` · `본문이 빈 뒤로 삼백 밀리초가 지나면 스피너가 나온다`
- 🔴 `새 달 조회가 실패하면 옛 달을 보여주다 실패 화면으로 한 번에 바뀐다` — 실패도 중간 상태 없이
  옛 짝에서 한 번에 넘어간다
- `전환 지연 동안 건수가 비지 않는다`

## 🔴 3차에서 새로 생긴 것 — 판단 필요 2건

1. **전환 실패 시 스낵바가 두 번 뜰 수 있다.** 실패하면 `selectedMonth` 를 이전 달로 되돌리는데
   (안 되돌리면 같은 방향 재탭이 `StateFlow` 동일값 필터에 걸려 **재시도가 통째로 죽는다** — 회귀
   테스트로 고정했다), 그 되돌림이 이전 달을 한 번 더 조회한다. **네트워크가 계속 죽어 있으면 그
   재조회도 실패해 탭 1회에 스낵바 2회**가 된다. 억제 로직은 넣지 않았다 — *"되돌림으로 시작된
   조회"* 를 구분하는 상태가 필요한데 그건 별도 판단이다
2. **상세를 보고 돌아올 때도 전환 경로를 탄다.** `WhileSubscribed(0)` 재구독이 `isFirstLoad == false`
   라, 갱신이 300ms 를 넘기면 **사용자가 탭하지 않았는데 건수에 `"불러오는 중"`** 이 뜬다.
   이전 데이터가 유지되는 쪽이 낫다고 보고 그대로 뒀지만 **의도된 부작용**임을 남긴다

✅ **테스트 공백 1건 해소** — `첫 로딩에서만 스피너가 나온다` 추가(pm-lead 판정). 월 전환에서
`Loading` 이 사라지며 **첫 로딩 전용 상태로 좁아졌다는 사실 자체**를 단언으로 고정한다. 다음 사람이
월 전환에 `Loading` 을 되살리면 여기서 걸린다 — **깜빡임이 그대로 돌아오는 경로**다.

🔴 **값이 아니라 방출로 단언한 것이 요점이다.** `uiState.value` 만 보면
*"탭 즉시 `Loading` → 300ms 에 힌트가 덮음"* 형태의 회귀가 **그대로 통과한다**(최종 값은
`Data` + `"불러오는 중"` 이라 단언이 전부 맞는다). turbine 으로 **탭 이후 방출이 힌트 하나뿐**임을
단언해 그 구멍을 막았다. `응답이 빠르면 중간 상태 없이 새 달로 바뀐다` 와 같은 패턴이다.

⚠️ 한계 하나는 남는다 — `triggerStateIn` 의 `initialValue` 자체가 `Loading` + `month == null` 이라,
첫 로딩 단언은 *"`monthFlow` 가 즉시 `Loading` 을 보냈다"* 까지는 구분하지 못하고 **화면에 보이는 값**만
고정한다.

## 🔴 이 feature 에서 반복된 패턴 — **앞선 개선이 다음 개선의 전제를 깬다**

40dp 배경이 ripple 을 덮은 건 **2차가 틀려서가 아니다.** 컨테이너 축 판단은 옳았고 비활성 구분도
그 덕에 명확해졌다. 다만 **부작용이 딸려 있었고**, 1차에는 라벨이 즉시 바뀌며 피드백을 지고 있어
보이지 않다가 **3차에서 눌림이 주역으로 승격되면서** 드러났다.

**같은 형태가 이 feature 에서만 여섯 번 반복됐다** — 기록해 둘 가치가 있다:

| 무엇이 | 무엇의 전제를 깼나 |
|---|---|
| 사용자의 월 이동 확정 | 1차의 *"전체 반환"* 근거 2개 |
| 월 이동 바 도입 | sticky 월 헤더의 존재 이유(*"월 이동 UI 를 뺀 대가를 갚는다"*) |
| 하한 폐기 | §2.5.1 CTA 근거(*"어느 달로도 갈 수 없다"*) · §2.5.1a-④ 거리 미표시의 사정 범위 |
| B형 고지 신설 | 내가 오용 방지로 좁힌 `Boolean` (월 값을 문장으로 출력해야 한다) |
| 2차 컨테이너 배경 | 기본 ripple (불투명 자식이 indication 을 덮는다) |
| 3차 전환 지연 | *"라벨 즉시 전환"* 이 지던 탭 피드백 · 테스트 불변식(*"즉시 비운다"*) |

🔵 **교훈은 "개선을 미루라"가 아니다.** 매번 **깨진 것이 결론인지 근거인지**를 갈라 처리한 것이
이 feature 가 무너지지 않은 이유다 — 근거만 죽고 결론이 사는 경우(하한 논거 → *"차단에서 고지로"*),
결론까지 뒤집히는 경우(불리언 → 절대값), 사정 범위만 좁는 경우(§2.5.1a-④)가 전부 달랐다.
🔴 **그리고 그 판정은 테스트에서 가장 날카롭게 드러난다** — 단언이 낡은 것과 **불변식 자체가 바뀐 것**은
다르고, 후자는 테스트를 고치는 게 아니라 **다시 쓰는** 일이다(3차에서 7건이 그랬다).

## Working tree 상태 (3차 시점)

🔵 **사용자가 1차 월 이동 구현을 커밋했다** — `09aa423 fix: 챌린지 히스토리 월별로 볼 수 있게 변경`.
**2·3차 개정분은 아직 uncommitted** 로 `:feature:mypage` **8파일**이다(commonMain 6 + commonTest 2).
브랜치 `main`, 내가 만든 커밋은 **0건**이다.

---

# 🔴 2차 개정 — 하한 폐기 + 비활성 시각 강화 (2026-08-31)

실기에서 나온 사용자 지시 2건. **계약 변경 0** — 응답 shape 이 하나도 안 바뀌어 wire 를 건드리지 않았다.

## 계기 — 내가 조사한 버그가 이 개정을 만들었다

실기 신고: *"이전/다음 달 눌러도 동작 안 한다."* 조사 결과 **앱 결함이 아니었다** — DB 의 종료
챌린지가 **전부 이번 달**이라 `최초 기록월(하한) == 이번 달(상한) == 선택월` 이 되어
**양쪽 화살표가 정당하게 비활성**이었다. 계약도 구현도 정확히 동작했는데, **그 정확함이 사용자에게는
고장으로 보였다.**

🔴 **더 나쁜 점**: 카드가 보이므로 빈 달 문구도 안 뜬다 — *왜 안 움직이는지 화면이 아무 말도 하지
않는* 상태였다. design 경계표는 "양쪽 비활성"을 **기록 0건** 행으로만 적어 뒀는데, 실제로 터진 건
**기록이 있는 사용자**였다(표에 없던 조합).

**조사 중 확인한 것 (읽기 전용 SELECT)** — 이 수치가 정책 판정을 바꿨다:
```
COMPLETED 7건 → 전부 2026-08 (이번 달)
가입: 이우건 2026-05-07 / 테스터1·2·3 2026-08-03
```
→ pm-lead 초안이던 **가입월 하한은 4계정 중 1개만 고친다.** 가입 첫 달 사용자는 정의상
`가입월 == 이번 달` 이라 **문제가 가장 자주 나는 집합에서 해법이 가장 약했다.** backend·design 도
독립적으로 같은 결론에 도달했고, **과거 끝 = 무제한**으로 확정됐다(`joinedMonth` 신설 폐기).

## 구현

| 지시 | 반영 |
|---|---|
| ① `<` 를 항상 활성 | `ArchiveMonthCursor` 에서 하한 비교 **삭제**. `canGoPrevious` 프로퍼티 자체를 없앴다(상수 true 를 남기지 않았다) |
| ② 비활성 시각 강화 | 화살표에 **컨테이너 축 신설** — 활성 `Box(40dp, radius 12dp, secondary)` + 아이콘 `onBackground` / 비활성 **배경 없음** + `onBackground.copy(alpha = 0.25f)` |
| ③ 빈 달 B형 | 첫 기록보다 이전 달이면 부제를 `"첫 기록은 {Y}년 {M}월이에요"` 로 교체 |
| ④ CTA 발동 축소 | `firstArchivedMonth == null` **&& 이번 달**. 기록 0건이어도 과거 달은 빈 달 화면 |

🔴 **②의 원인이 농도가 아니라 신호 개수였다** — 앱의 비활성 관례(`Button.kt:29-30`)는 **두 축**
(`disabledContainerColor` + `disabledContentColor`)인데 화살표엔 **컨테이너가 없어서** content 축만
복사돼 있었다. 버튼은 배경째 흐려지며 "못 누른다"를 말하는데 화살표는 아이콘만 옅어졌다.
배경을 만들고 비활성에서 없애니 *"버튼처럼 보이는 것"이 활성에만 존재*하게 된다.
🔴 **터치 48dp·좌우 8dp·바 높이 64dp·슬롯 기하는 전부 무변경** — 배경 40dp 는 **시각 신호이지 터치
영역이 아니다**(48dp `IconButton` 안에 얹은 그림).

## 🔴 내 판단 하나를 되돌렸다 — `Boolean` → `YearMonth?`

하한이 사라지자 `firstArchivedMonth` 의 역할이 **빈 상태 2종 판별 하나**로 좁아졌다. 그래서 커서에서
빼면서 **`hasArchivedRecords: Boolean`** 으로 좁혔다 — 근거는 *"월 값이 남으면 다음 사람이 그걸로 다시
하한을 만든다"* 였고, `Boolean` 이면 그 오용이 타입 수준에서 불가능하다.

**그 뒤 design 이 B형 부제를 신설하면서 되돌렸다.** `"첫 기록은 2026년 5월이에요"` 는 **월 값을 문장으로
출력**해야 하고 비교(`선택월 < firstArchivedMonth`)도 필요해서 `Boolean` 으로는 표현 자체가 안 된다.

🔴 **오용 방지를 포기한 게 아니라 수단을 옮겼다** — 화면은 `showsNoRecordsEver` ·
`emptyMonthSubtitle` **계산 프로퍼티만 읽고** 원시 비교를 하지 않는다. `ArchiveScreen` 안에
`firstArchivedMonth` 라는 식별자가 **한 번도 나오지 않는다** — 그 월로 `<` 를 막으려면 먼저 새 프로퍼티를
뚫어야 한다. 여기에 KDoc 경고와 회귀 테스트를 더해 3중으로 지킨다.

## 🔴 `ArchiveMonthCursor` 설계가 값을 한 지점

1차에 이 타입을 *"그 달에 데이터가 있는지 아예 모르게"* 만든 것이 이번 개정을 **하한 삭제 하나로**
끝냈다 — 데이터를 참조하는 경로가 애초에 없었다. 다만 pm-lead 가 *"한 곳만 바뀐다"* 로 가정한 것은
**세 곳이 맞았다**: 커서(이동 판정) 외에 `ArchiveScreen` 이 같은 필드를 **빈 상태 판별**로도 읽고
있었다 — **계약의 `firstArchivedMonth` 중첩이 앱에도 거울처럼 있었다.** 그래서 역할을 쪼갰다.

## 부수 — 신고된 버그가 구조적으로 사라진다

`<` 가 항상 활성이라 **"양쪽 동시 비활성" 조합이 발생 자체가 불가능**해졌다. 1차에 *"경계표에 없던
조합"*, 2차에 *"이름만 갈아입고 남는다"* 고 짚었던 그 행이 이번엔 **없어진다.** 남는 비활성은
`>` 하나뿐이고 의미도 하나(미래 없음)뿐이다.

⚠️ **`alpha 0.25f` 는 실기 확인이 필요한 유일한 수치다** — 코드로 검증할 수 없다. 0.2 미만은
다크에서 사실상 숨김이라 금지선이다.

⚠️ **"무제한"은 타입상 완전한 무한이 아니다** — `YearMonth.minusMonth()` 가 약 -999,999,999년에서
예외를 던진다. 탭으로 도달하려면 **120억 번**쯤 눌러야 해서 **방어 코드를 넣지 않았다**(관측된
버그도 도달 가능한 입력도 아니다). 사실만 기록한다.

## 변경된 파일 (2차) — 전부 `:feature:mypage`, wire 0

- `contract/ArchiveMonthCursor.kt` — `earliest` 삭제(2필드로 축소), `canGoPrevious` 삭제,
  `previous()` 무조건 이동, `isLatest` 추가
- `contract/ArchiveState.kt` — `ArchiveUiState.firstArchivedMonth: YearMonth?` +
  `showsNoRecordsEver` · `emptyMonthSubtitle` 계산 프로퍼티
- `component/ArchiveMonthNavBar.kt` — `MonthArrowButton` 분리, 컨테이너 축 신설
- `component/ArchiveEmptyMonth.kt` — `subtitle` 파라미터화(A형/B형), 프리뷰 2종
- `ArchiveScreen.kt` — 빈 상태 3분기, 프리뷰 추가(B형 / 기록 0건 + 과거 달)
- `ArchiveViewModel.kt` — `firstArchivedMonth` 전달, `initialValue = null`
- 테스트 2파일

🔵 **child 가 낡은 근거 2건을 스스로 정정했다** — `ArchiveEmptyMonth` KDoc 의 *"방향을 지정하면
하한 근처에서 거짓이 된다"* 가 **폐기된 하한을 근거로 삼고 있어 B형과 정면으로 어긋났고**,
`ArchiveMonthNavBar` 프리뷰 설명 2개가 *"더 거슬러 올라갈 수 없다"·"양쪽이 모두 막혀 살아있는
조작이 없다"* 로 **이번에 없앤 상태를 현재 사양처럼** 서술하고 있었다.

## 테스트 결과 (2차) — 실측 XML

**전체 Android 565 / iOS 480. `failures=0 errors=0` (전 모듈 XML 전수 확인).**
기준선 563 / 478 → **+2 / +2**(`:feature:mypage` 49 → **51**, 양 타깃 동일). **회귀 0.**

⚠️ **전체 실행의 gradle 종료 코드는 1이다.** 실패 4건은 **전부 선재 이슈**(`:local:database` KSP +
백틱 이름 때문에 iOS 테스트가 컴파일되지 않는 3개 모듈)이고 **1차 마감 때와 정확히 같은 집합**이다.
🔴 **테스트 실패는 전 모듈 XML에 한 건도 없다.**

🔵 **§안티패턴 절이 이번에도 걸렸다** — 백그라운드 완료 알림은 `exit code 0` 을 보고했지만
`GRADLE_EXIT=1` 이었다(알림이 보는 것은 셸의 코드다). 로그의 `BUILD FAILED` 문자열과 대조해서
선재 실패임을 확인했다. **같은 패턴의 네 번째 확인**이다.

### 신규 테스트가 고정한 것

- 🔴 `기록이 전부 이번 달이어도 이전 달로 갈 수 있다` — **신고된 버그의 회귀 테스트.**
  `firstArchivedMonth == 이번 달`(= 옛 하한 == 상한)인 상태에서 실제로 직전 달을 조회한다.
  하한이 되살아나면 **이 테스트가 가장 먼저 깨진다**
- `기록이 하나도 없어도 이전 달로 갈 수 있다` (VM) · `기록이 없어도 이전 달로 계속 갈 수 있다` (커서)
- `기록 0건이어도 과거 달에서는 CTA 화면이 아니다` — 이번 달 `true` / 이전 달 `false` 를 둘 다 단언
- 🔴 `첫 기록보다 이전 달이면 부제가 첫 기록 월을 알린다` — B형 `"첫 기록은 2026년 5월이에요"` 와
  구간 안 A형 문구를 **한 테스트에서 짝으로** 고정
- `이번 달에서만 isLatest 가 참이다` (커서)

프리뷰도 함께 늘렸다 — B형 빈 달, 기록 0건 + 과거 달(CTA 없음), 화살표 2단 시각.
🔴 **도달 불가능해진 프리뷰 2개는 지우지 않고 남겼다** — *"지금은 만들 수 없는 상태이며 다시 만들지
않기 위한 기록"* 으로 KDoc 을 고쳤다(컴포넌트의 경계 표현 능력도 함께 붙든다).

## Working tree 상태 (2차)

- **브랜치** `main`, **커밋 0건.** 변경분 staged/unstaged 그대로 — 사용자 처리 영역
- 코드 변경은 `:feature:mypage` **7파일**뿐. wire·서버 계약 무변경
...
## 🔴 셸이 내 문자열을 먼저 읽는다 — 세 변종이 나왔다. 네 번째를 만들지 마라

**같은 뿌리의 세 변종이 이 feature 에서만 세 번 나왔다.** 뿌리는 gradle 이 아니라 **셸**이다 —
🔴 **셸에 넘기는 문자열은 셸이 자기 문법으로 먼저 읽는다.** 내가 의도한 것과 셸이 읽은 것이
갈리는 순간 명령의 의미가 바뀐다.

| # | 구문 | 셸이 무엇을 가로채나 | 증상 | 언제 |
|---|---|---|---|---|
| 1 | `./gradlew … \| tail -80` | **종료 코드** — 파이프라인의 코드는 마지막 명령(`tail`)의 것 | 🔴 조용히 성공으로 보인다 | 1차 (2026-08-26) |
| 2 | `./gradlew … ; echo "EXIT=$?"` | **종료 코드** — 셸이 반환하는 건 마지막 명령(`echo`)의 것. `$?` 는 stdout 에만 남고 **상위 도구가 보는 코드는 항상 0** | 🔴 조용히 성공으로 보인다 | 2차 (이번, mobile) |
| 3 | `git commit -m "… $? …"` | **문자열 자체** — 인용부 안 `$?` 를 확장해 인자가 쪼개진다 | ⚠️ 시끄럽게 실패한다(`pathspec … did not match`) | 3차 (이번, pm-lead) |

🔴 **1·2 와 3 의 차이가 중요하다.** 앞 둘은 **조용히 성공으로 보이고**, 3 은 **즉시 터진다.**
그래서 **3 이 훨씬 낫다** — 터지는 실패는 발견되고, 조용한 성공은 리포트에 그대로 실린다.
같은 뿌리인데 위험도는 반대다.

### 처방

**두 번 다 같은 방법으로 잡았다(1·2)** — 🔴 **로그 본문의 `BUILD SUCCESSFUL`/`BUILD FAILED`
문자열을 직접 grep 했다.** 그게 gradle 자신이 쓴 사실이고 셸 구문이 손댈 수 없는 유일한 값이다.
(1차에서는 XML timestamp 가 실행 시각보다 과거인 것으로도 교차 확인했다.)

| 상황 | 처방 |
|---|---|
| 빌드·테스트 성패 판정 | ① 로그 파일로 리다이렉트하고 **파이프를 물리지 않는다** ② `BUILD SUCCESSFUL` 을 grep ③ 결과 XML 의 `failures`/`errors` **와 timestamp**. 🔴 **①만으로는 부족하다** — 2차 함정이 정확히 *"①은 지켰는데 종료 코드가 `echo` 의 것"* 이었다 |
| 특수문자가 든 긴 문자열(커밋 메시지 등) | 🔴 **`-m` 이 아니라 `-F <파일>`.** `$` `` ` `` `!` 가 들어가는 순간 인용부는 방어가 되지 못한다 |
| 백그라운드 실행 결과 | ⚠️ **완료 알림의 "exit code 0" 도 셸의 코드다.** 그것만 믿지 마라 |

⚠️ 이번에 이 대조로 **작업 전 기준선 실행이 실제로는 실패(`GRADLE_EXIT=1`)였다는 것**을 잡았다.
그걸 놓쳤다면 아래 §선재 이슈 4건을 **내가 만든 회귀로 오인**했거나, 반대로 "기준선도 통과했으니
문제없다" 로 넘어갔을 것이다.
...
3. 🔴 **말일 챌린지 축의 자동화 공백 — 내가 지운 방어의 대체물이 절반만 있다.**
   앱측 `challengeDate` 버킷팅(`toArchiveMonths`)을 지우면서 *"이제 서버가 유일한 방어선"* 이라고
   했는데, backend 회신으로 그 방어선의 실제 강도가 확인됐다:
   - ✅ `ChallengeHistoryServiceTest.판정 시각이 아니라 챌린지 날짜로 월을 자른다` 가 **양방향으로**
     고정한다(`challengeDate=7/31, completedAt=8/1 00:05` 건이 **7월에 있고 8월에 없다**). 현재 pass
   - ⚠️ **그런데 fake 를 태우는 단위 테스트라 JPQL 축을 못 덮는다** — fake 가 실제 구현과 같은
     규칙을 다시 구현할 뿐이라 **구현과 fake 가 함께 틀리면 통과한다**
   - ⚠️ 실제 SQL 축은 throwaway DB **수동 실구동으로만** 확인됐다(자동화 없음). Docker 부재로
     통합 테스트 49건이 상시 skip 인 기존 공백과 같은 자리이며, pm-lead 가 백로그의 컨테이너
     런타임 항목에 **"설치 시 1순위 자동화 대상"** 으로 승격시켰다
   - 🔴 **깨졌을 때의 증상이 조용하다** — 월 필터가 `completed_at` 으로 어긋나면 카드가 응답에서
     사라지고 화면은 *"이 달엔 끝낸 챌린지가 없어요"* 로 **정상처럼 보인다.** 앱에 감지 수단이 0이다.
   backend 가 *"테스트로 고정했다고만 답하면 네가 실제보다 안전하다고 믿게 된다"* 며 먼저 밝혀 왔다
4. **상한(이번 달)은 세션 중 고정** — 첫 응답 에코를 들고 있으므로, 앱을 열어 둔 채 자정을 넘겨
   달이 바뀌면 새 달로 못 간다. 재진입하면 해소된다(ViewModel 재생성). 방어 코드를 넣지 않았다.
   ✅ **design 이 내 보고에 없던 근거를 도출해 수용 판단을 강화했다(§2.1.2 "알려진 한계")** —
   🔴 **막히는 그 달은 최소 하루 동안 어차피 비어 있다.** `challengeDate` 가 오늘/내일이고
   `COMPLETED` 는 자정 배치 판정이라, 9/1 00:01 에 방금 판정된 건은 **전부 8/31 자**이고 9/1 자
   챌린지는 **9/2 00:0x 에야** `COMPLETED` 가 된다. 즉 *"기록에 도달 못 하는 버그"* 가 아니라
   *"빈 달에 못 가는 상태"* 다.
   ⚠️ **훗날 고칠 때의 방향도 못박혔다 — 시계가 아니라 서버에 묻는다**(포그라운드 복귀 시 재조회로
   `month` 에코 갱신). 기기 시계를 다시 들이면 §2.1.2 가 없앤 실패 경로가 되살아나는데, 그 실패는
   **잘못된 화면을 정상처럼** 보여주고 이 한계는 **재진입 한 번**으로 풀린다 — 교환비가 나쁘다
5. **문구 미확정 승계** — 1차의 §6 디자이너 확인 대상에 §6-⑰(월 이동 바 형태·건수 이관·라벨
