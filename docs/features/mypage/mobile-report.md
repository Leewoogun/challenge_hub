...
## 🔴 검증 명령이 검증을 무력화한 패턴 — 두 번 나왔다. 세 번째를 만들지 마라

**같은 함정의 두 변종이 1차와 2차에서 한 번씩 나왔다.** 둘 다 *"gradle 의 종료 코드가 나에게
도달하지 못하게 만드는 셸 구문"* 이고, 증상이 **"실패한 빌드가 성공으로 보인다"** 로 같다.

| # | 구문 | 무슨 일이 일어나나 | 언제 |
|---|---|---|---|
| 1 | `./gradlew … \| tail -80` | 파이프라인의 종료 코드는 **마지막 명령(`tail`)** 의 것이다. gradle 이 실패해도 `tail` 은 성공한다 | 1차 (2026-08-26) |
| 2 | `./gradlew … ; echo "EXIT=$?"` | 셸이 반환하는 것은 **마지막 명령(`echo`)** 의 코드다. `$?` 값은 stdout 으로 나가지만, **상위 도구가 보는 종료 코드는 항상 0** 이다 | 2차 (이번) |

🔴 **일반화: gradle 뒤에 무엇이든 붙으면 종료 코드의 소유자가 바뀐다.** 파이프든 세미콜론이든
같다. "빌드 결과를 보기 좋게 만들려는 시도" 가 매번 결과 자체를 가린다.

**두 번 다 같은 방법으로 잡았다** — 🔴 **로그 본문의 `BUILD SUCCESSFUL`/`BUILD FAILED` 문자열을
직접 grep 했다.** 그게 gradle 자신이 쓴 사실이고, 셸 구문이 손댈 수 없는 유일한 값이다.
(1차에서는 XML timestamp 가 실행 시각보다 과거인 것으로도 교차 확인했다.)

**권장 절차 3단**: ① 로그 파일로 리다이렉트하고 파이프를 물리지 않는다 ② `BUILD SUCCESSFUL` 을
grep 한다 ③ 결과 XML 의 `failures`/`errors` 와 **timestamp** 를 본다. ①만으로는 부족하다 —
이번 2차 함정이 정확히 "①은 지켰는데 종료 코드가 `echo` 의 것이었던" 경우다.
