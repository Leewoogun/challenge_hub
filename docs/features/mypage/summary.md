...
2. ~~**보관함 = 전체 최신순 + 월 섹션 헤더** (월 파라미터 아님 — 빈 달 문제 + 홈 7일 이후 도달이
   ...
   **수용 기준 정정**: *"홈 7일 이후 결과에 **무조작** 도달"* → **"월 이동 1~2회로 도달"**
   ...
   제외는 유지.
   🔴 **2026-09-01 3차 — 실기 후 하한 폐기**: 종료 기록이 전부 이번 달이라 **하한=상한=이번 달**이 되어
   양쪽 화살표가 동시에 비활성 → 사용자가 "고장"으로 읽었다. → **과거 무제한**(`<` 항상 활성).
   ~~가입월 하한~~ 안은 3개 에이전트가 각각 실측으로 반박해 폐기(실기 4계정 중 3명이 가입월=이번 달이라
   **고치려는 사고를 못 고친다**). 🔵 **"양쪽 동시 비활성"이 구조적으로 발생 불가능**해졌다.
   `firstArchivedMonth` 는 존치하되 용도가 **빈 상태 2종 판별**로 축소(+"이 값으로 `<` 를 막지 마라"
   금지를 계약·KDoc·회귀 테스트 3중으로). **비활성 시각 강화** — 원인이 농도가 아니라 **신호 개수**였다
   (앱 버튼 관례는 배경+아이콘 2축인데 화살표엔 배경이 없었다) → 활성일 때만 배경. 서버 코드 변경 **0**
   (하한은 앱이 스스로 걸던 규칙이었다). 상세는 [spec.md §후속 개정](./spec.md) · [change-log.md](./change-log.md)
...
- [ ] ⚠️ **비활성 화살표 `alpha 0.25f` 실기 확인** — 코드로 검증 불가한 유일한 수치. 0.2 미만은 다크에서
  사실상 숨김이라 금지선(design 확정). 조정 필요 시 design 이 문서를 맞춘다
- [ ] **월 이동 잔여** — ① 상한이 세션 중 고정(앱을 켜 둔 채 자정을 넘기면 새 달로 못 감. 재진입 시 해소 —
  *"가정에 방어 로직 금지"* 로 코드 미추가). 🔵 **심각도 낮음이 실측으로 확인됨** — 판정이 자정 배치라
  `9/1 00:01` 에 판정된 건은 전부 `8/31` 자이고 **9/1 자 기록은 9/2 새벽에야 `COMPLETED`** 가 된다.
  즉 *"기록에 도달 못 하는 버그"* 가 아니라 *"최소 하루 동안 어차피 빈 달에 못 가는 상태"* 다.
  고칠 때는 시계가 아니라 **서버에 다시 묻는다**(포그라운드 복귀 시 재조회 → `month` 에코 갱신 —
  design §2.1.2 에 방향 명시) / ② 연타 시 300ms 지연 스피너가 명멸을 없애는지는 실기 확인
