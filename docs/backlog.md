...
- **마지막 갱신**: 2026-07-02 (friends 2차 친구 추가 완료 — 백엔드 7 endpoint + 모바일 도메인/Data/ViewModel/UI + KakaoLink 초대 SDK Default TextTemplate. iOS SPM KakaoSDKShare / iOS 유닛테스트 / 원격 이미지 로더 / IconTextButton iconSize 후속 등재)
...
| `TokenLocalDataSource` / `AppSettingsLocalDataSource` interface 분리 (테스트 추가 시) | [user-info/mobile-report.md](./features/user-info/mobile-report.md) | mobile | 본 작업에서 `UserInfoLocalDataSource`만 interface 분리. 다른 LocalDataSource는 테스트 추가 시 동일 패턴 검토. |
