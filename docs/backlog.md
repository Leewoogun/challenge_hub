...
- **마지막 갱신**: 2026-08-03 (`dev-test-login` partially-completed — 테스트 계정 3개로 친구 요청→수락 실사용 검증 성공. 백엔드 134/134 + 켠 상태 실서버 34/34, 모바일 148/148. 🔴 격리 OFF 실서버 검증 미완 / 컨테이너 런타임 부재로 통합 45건 미실행 / PM 레포 커밋 대기)
...
| **위저드 시스템 백 버튼으로 step1 → step0 복귀 미구현** (`PlatformBackHandler`를 `:core:ui`로 승격) | [challenge-create/summary.md](./features/challenge-create/summary.md) | mobile | `PlatformBackHandler`가 `:feature:main`에 expect/actual로 갇혀 `:feature:challenge:create`에서 못 쓴다. 현재 TopBar 뒤로가기만 단계 전환하고 시스템 백은 위저드 전체를 pop. |
